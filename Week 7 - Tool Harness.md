...
> **Cross-stack convergence (deer-flow).** This is not a Claude-Code-only idea. bytedance/deer-flow ships the identical mechanism as a `ToolErrorHandlingMiddleware`: a tool raise is caught and reframed into a model-facing message — *"Error: Tool 'task' failed with TypeError: …. Continue with available context, or choose an alternative tool."* — which the agent reads and recovers from (retry, switch tools, or replan) instead of dying. Three independent production harnesses converging on "errors are data on the main path" — Claude Code (`query.ts`), Codex (`execpolicy`), and deer-flow (`ToolErrorHandlingMiddleware`) — is the strongest signal that this is a load-bearing pattern, not a stylistic choice. And note *where* deer-flow puts it: a **middleware**, not a per-tool `try/except` — the same point Concept 1's gate-sequence and `_execute_with_guards()` make. See [[Engineering Decision Patterns]] Pattern 40 (tool-error-as-observation) and Pattern 41 (cross-cutting concerns as middleware + provider seam).
...
- **bytedance/deer-flow (2025).** github.com/bytedance/deer-flow. Production long-horizon "SuperAgent" harness (sandboxes, subagents, tools, skills, message gateway). The convergent **third** source — alongside Claude Code (`query.ts`) and Codex (`execpolicy`) — for two of this chapter's core concepts: **tool-error-as-observation** (`ToolErrorHandlingMiddleware`: a tool raise reframed into a model-facing recovery message → Concept 4 / [[Engineering Decision Patterns]] Pattern 40) and **cross-cutting concerns as middleware + provider seam** (`guardrails/{middleware,provider}.py`, `sandbox/middleware.py`: enforcement wrapped uniformly behind a provider, swap-by-config → Concept 1 / Pattern 41). Independent convergence of three production harnesses is the evidence these concepts are load-bearing, not stylistic.
...
- **Foreshadows:** W11 System Design — `Tool` dataclass with governance fields scales into production tool registry with versioning, capability routing, multi-tenant budget accounting. A2A agent card from §7.X becomes building block for agent registry. W12 Capstone — 20-scenario pass/fail matrix and local-vs-cloud reliability delta are empirical anchors capstone cites. W7.6 Small-Model Agent Stacks — this single-model harness generalizes into multi-model small-model orchestration (MagenticLite / Fara); the per-tool governance fields carry over to the delegate layer.
