...
### Scope decision — why tree-index / GraphRAG are not comparison components in this lab

A reader who has just finished W2.7 (PageIndex / tree-index RAG) and W2.5 (GraphRAG) may reasonably ask: "shouldn't W3 evaluate tree-index and graph alongside vector to give a four-way comparison?" The answer is **no — deliberately**, for three structural reasons that are worth understanding because they illustrate when *not* to add a comparison component:

1. **W3's corpus is the wrong shape for tree-index.** The lab's `data/docs.jsonl` is flat, single-paragraph passages — Wikipedia / MS MARCO / synthetic-QA style. Tree-index requires a hierarchical document with table-of-contents structure (10-K filings, contracts, regulations, technical specs). There is no hierarchy to build a `tree.json` from in a flat-passage corpus. You could synthetically cluster passages into a fake hierarchy, but that's a different exercise — and it would test the cluster-quality, not the tree-index pattern. Tree-index is the right tool when the document IS the structure; W3's corpus has chunks, not structure.

2. **W3's recall is already saturated, leaving no headroom for any new retrieval strategy.** Baseline `context_recall = 1.000` on this dev set. Tree-index, GraphRAG, hybrid retrieval, HyDE, multi-query — all of these are *recall-augmentation* strategies. Adding any of them to a saturated system can only tie or hurt; they cannot win on a recall-saturated benchmark. HyDE was rejected for exactly this reason (Bad-Case Entry 1); multi-query was rejected for exactly this reason. Tree-index would be rejected for the same reason. Ship the rejection logic once (HyDE) and generalize the lesson: "when context_recall is already 1.000, no recall-augmentation strategy can lift; switch corpora or add a retrieval-quality dimension before testing more strategies."

3. **W2.7 already serves the cross-architecture comparison function.** [[Week 2.7 - Structure-Aware RAG#4.3.2 PageIndex optimization run — tree judge 0.44 → 0.79 (2026-05-07)|W2.7 §4.3.2]] is the canonical multi-architecture eval lab in this curriculum: vector + graph + tree on the same Berkshire 2023 corpus, scored with the same RAGAS-style judge + substring helpers (`score_substring` and `score_llm_judge` reused from `lab-02-5/src/compare.py`). Replicating that comparison in W3 with a different corpus would either (a) reproduce the W2.7 finding (tree wins on long structured docs), or (b) show tree losing on a flat-passage corpus where it doesn't fit. Both outcomes are already implicit in W2.7's "route by query/corpus shape" insight — adding W3 numbers would be redundant.

**The scope-discipline lesson — what W3 IS:** an eval-framework chapter. The artifact is the RAGAS pipeline + the dev set + the regression-test contract. Its retrieval-strategy comparisons (HyDE, multi-query) are *within-architecture lifts on top of vector*, not architecture choices. The architecture choice is upstream of W3 and lives in W2/W2.5/W2.7. Saying "W3 also compares tree-index" would muddy W3's load-bearing teaching point: that quality measurement is the eval framework's job, regardless of which retrieval architecture you're measuring.

**The cross-cutting lesson — when to add a comparison component:** when (a) the corpus shape supports it, AND (b) the metric has headroom, AND (c) the comparison is not already covered by an upstream chapter. W3 fails on (a) and (b); the comparison is satisfied by (c) via W2.7. Drop it from this lab; reference W2.7 for readers who want it.
...
Save as `src/05_trace.py`. Refactored 2026-05-07 from inline 30-line OTel ceremony to a single `trace_run(...)` call from `shared/phoenix_tracing/`. The shared lib does cached idempotent `register()` setup on first call + explicit `OpenAIInstrumentor` + `LangChainInstrumentor` + parent span with `pipeline.variant` attribute + auto-instrumented children nesting under the parent. Adding tracing to a future lab is now `from phoenix_tracing import trace_run` + one call site, not 30 lines of imports + setup.

```python
"""Re-run baseline + HyDE + multi-query, emitting OpenTelemetry traces to Phoenix.

Refactored to use shared/phoenix_tracing/trace_run helper — see that
package's README.md for the API surface (init_phoenix / phoenix_span /
trace_run for three ergonomic levels).
...
- **Distinguish from:** cross-architecture retrieval comparison — W3 evaluates a *single retrieval pipeline* (baseline vector + rerank) against quality metrics; the eval framework is the artifact, not the architecture choice. For "vector vs graph vs tree-index on the same corpus + eval set" head-to-head, see [[Week 2.7 - Structure-Aware RAG#4.3.2 PageIndex optimization run — tree judge 0.44 → 0.79 (2026-05-07)]] — W2.7's three-way comparison is the canonical cross-architecture eval lab. W3's HyDE + multi-query rejections (both lifts on top of vector) are within-architecture; the cross-architecture lift question lives in W2.7.
- **Connects to:** W9 Faithfulness Checker (post-hoc validation operationalizing the faithfulness metric as a production service); W3.7 Agentic RAG (RAGAS provides eval baseline comparing single-pass vs 5-node agentic); [[Week 2.7 - Structure-Aware RAG]] (uses RAGAS-style judge + substring scoring — `score_substring` and `score_llm_judge` from `lab-02-5/src/compare.py` — to score the three-way tree/graph/vector comparison on Berkshire 2023).
...
- **Cited by:** chapters that reference this chapter as a prerequisite or build-on; reverse links per Pattern 21 (Bidirectional Cross-Reference Invariant):
  - **W11.8**: Continuous Training — RAG eval metrics feed the CT pipeline's drift detector
  - **W2.5**: GraphRAG — graph-based retrieval needs RAG eval methodology to compare against vector baselines
  - **W4**: ReAct — agent loops invoke retrieval; RAG eval is the quality gate on retrieved evidence
  - **W6.85**: Prompt Templates — retrieval-prompt templates are evaluated using RAG eval methodology
  - **W7.3**: Production LLM Infrastructure — RAG eval is part of the staged eval pipeline before production rollout
