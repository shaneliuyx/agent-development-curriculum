...
"I'd use two tiers: an operational tier for fast-write coordination — atomic claims, scroll handoff, current task state — and a semantic tier for slow-write durable knowledge, connected by a periodic consolidation pipeline. The split maps to hippocampus-neocortex separation: fast short-term coordination feeding a slow-write long-term store, with consolidation as the engineering equivalent of REM sleep. In my lab I wired a Go MCP server as the operational tier and a Postgres-backed LTM service as the semantic tier. The consolidation pipeline is where most production implementations fail — synchronous writes collapse the tier separation, and missing idempotency causes duplicate semantic facts. The two-tier composition materially outperformed either tier alone, with the biggest gap on cross-session, cross-agent recall where neither single-tier approach could bridge both dimensions."

*Principle:* tier separation keeps each system specialized; the consolidation pipeline — not the storage engines — is the load-bearing component.

**Soundbite 2 — *"What did you learn building a consolidation pipeline?"***

"The deepest lesson was that lifecycle position is load-bearing, independent of the primitive itself. My atomisation code — extracting subject-attribute-value triples — is correct. Applied at write time during consolidation on conversational data, accuracy collapsed to near-zero: my few-shot prompts were scenario-bound to task scrolls, so conversational events got either paraphrased into wrong-domain language or silently skipped, and no downstream stage could recover what was destroyed. Applied at read time — same code, after retrieval, with the actual question in hand — accuracy lifted uniformly across every compose model I tested. Same code, opposite outcome, because write-time errors compound through every downstream stage while read-time errors are recoverable in the same LLM turn. The rule I carry out: most 'compress at write' habits are left over from log-processing pipelines where ingest vastly outnumbers queries — agent memory is the opposite shape."

*Principle:* the write-time primitive either preserves or destroys the signal; resolve — deduplicate, rank, atomise — at read time where you have the question in hand.

**Soundbite 3 — *"When would two-tier memory be the wrong choice?"***

"Three cases. First, single-agent paraphrase-shaped queries — single-tier vector RAG is simpler and sufficient; the operational tier earns its cost only when multiple agents need to coordinate. Second, when the agents genuinely don't share knowledge — if agent A's experience has no value to agent B, the semantic tier is pure overhead. Third, when ingest data shape doesn't match the consolidation pipeline's assumptions — my lab confirmed this directly: a pipeline calibrated on structured task scrolls silently degrades on conversational data. The right check before committing to two-tier is to measure cross-over on a known-answer eval, not to assume the pipeline transfers."

*Principle:* two-tier is justified by shared knowledge across agents and query volume that exceeds what raw-scroll retrieval handles; audit both before paying the operational cost.
