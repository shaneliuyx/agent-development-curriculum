...
- **Block 2 — `touch()` separates the visit signal from ranking.** `metacog_recall.recall()` is pure-read by *default*; heat needs a *write* (which facts got injected). `touch()` is the wire-in — and it is now wired: `recall(track=True)` calls it on the IDs it returns, and `recall_block()` (the live agent loop's entry point) defaults `track=True`, so real recalls accrue heat automatically; `recall(track=False)` stays read-only for tests/ablation. The lazy import inside `recall()` avoids the cycle (heat_eviction imports metacog_recall) — a top-level *reverse* import would deadlock at module load, so deferring resolution to call-time (when both modules are fully loaded) is the standard escape hatch for a legitimate two-way dependency. The demo still calls `touch()` directly to simulate a long usage history without running hundreds of agent steps.
...
- **The read/retention loop is now closed.** `recall_block` reads a fact AND records the visit (`track=True`) → heat accrues → `enforce(budget)` dedups + evicts by heat → bounded store → cheaper, fresher reads. Before the wiring, heat was demo-only: eviction ran on simulated visits, so the store's retention had no connection to what the agent actually used. Closing the loop is what makes eviction *earned* rather than arbitrary — the facts that survive are the ones recall kept reaching for. (Generalized as [[Engineering Decision Patterns#Pattern 36 — Close the Read/Retention Loop (the read path records usage; retention consumes it)|Pattern 36]].)
`─────────────────────────────────────────────────`
