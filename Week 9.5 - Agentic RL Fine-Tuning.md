...
- **Distinguish from:**
  ...
  - **W5.5**: Training-Free GRPO ([[Week 5.5 - Metacognition]]) reuses this chapter's group-relative advantage $\hat{A}_i = (r_i - \mu_{\text{group}})/\sigma_{\text{group}}$ but consumes it in a textual experience buffer (no weight updates) instead of an optimizer. Weight-free selection of existing behaviors; the cheap rung below the weight-GRPO this chapter implements.
