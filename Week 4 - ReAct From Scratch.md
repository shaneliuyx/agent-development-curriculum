...
**Two implementation paths — text-parsing vs `tools=` API.** The 2022 ReAct paper's reference implementation is **text-parsing**: the model emits `Thought: ... / Action: name[arg]` as plain text, the harness halts generation on a `stop=["Observation:"]` sequence, parses the action with a regex, and feeds the tool result back as a `user`-role message prefixed with `Observation:`. The modern production path — what THIS chapter implements — is the **`tools=` API**: the model emits structured `tool_calls` in the response object, the harness reads them directly, and tool results come back as `role: tool` messages. The two paths solve the same problem with different load-bearing primitives. Text-parsing wins on local-model portability (works on any model that can follow output-format instructions, regardless of training); the `tools=` API wins on production reliability (no parser failures, no stop-sequence misfires, structured arguments). The choice is workload-driven, not philosophical — pick text-parsing when targeting local Ollama / MLX models that don't honor the `tool` role cleanly; pick `tools=` API when targeting Claude / GPT / Gemini production endpoints. See the canonical text-parsing implementation in `rulyone/Simple-ReAct-Agent` (§References) for the five-idiom reference: server+client dual stop defense, Observation-as-user-message, error-as-observation, `finish` sentinel separation, AST-walk `calculate` security pattern.
...
**Soundbite 1 — why ReAct over Chain-of-Thought (anchored to the observable run).** I built the loop frameworkless and ran it end-to-end: "sqrt 144, verify with `python_repl`" logged one SQLite row per step — iter 0 a `tool_call` at 31 ms, iter 1 the final answer, zero tool errors. That's ReAct's edge over Chain-of-Thought: tool results are real and land in the scratchpad as ground truth, so every conclusion traces to a logged function call, not a plausible fabrication. CoT only imagines what a search would return; ReAct makes the world load-bearing, and the audit sidecar proves it.

**Soundbite 2 — tool calling is a model × server-parser pairing, not a model property.** My most useful finding from probing the local fleet: on oMLX, `Qwen2.5-Coder` formed *correct* tool calls but returned empty `tool_calls` on **both** the OpenAI and Anthropic surfaces — the server had no parser for that family, so the call leaked into `content` as text and a client-side fallback had to recover it. Meanwhile the reasoning-distilled models scored `tool=1.00` yet `json` and `instr` at `0.00`, their `<think>` blocks eating the format. So I probe each model × server pair on every dimension — I never assume tool support from model size or a benchmark.

**Soundbite 3 — trust the harness less than the model.** My probe reported `reason=0` for models that were clearly doing the arithmetic. The bug was mine: a 64-token cap clipped verbose-but-correct derivations, so `finish_reason` was `"length"`, not `"stop"`. Raising it to 512 recovered the scores to 0.83. The discipline I carry from that: when a known-capable model scores implausibly low, I check `finish_reason` before blaming the model — a stingy token cap silently turns a correctness test into a brevity test, and the whole bad-case suite (16/16) only means something if the harness measures what it claims to.
...
- **rulyone (2026).** *Simple-ReAct-Agent.* https://github.com/rulyone/Simple-ReAct-Agent. Canonical 2022-paper text-parsing ReAct in ~285 LOC on Ollama + Llama 3.2. Five idioms worth knowing: (1) server-side `stop=["Observation:"]` + client-side `ACTION_RE` regex as belt+suspenders for the "model writes one Action then halts" contract; (2) Observation-as-user-message (per 2022 paper, predates the modern `tool` role); (3) tool exceptions become `Observation: Error from X: <exc>` so the model reacts instead of crashing the loop — same error-on-main-path discipline this chapter's Concept 5 names; (4) `finish[answer]` as sentinel NOT in the TOOLS dict — clean separation between tool-call actions and loop-end actions; (5) AST-walk `calculate` with whitelisted operator dispatch (never `eval()` LLM output). This chapter takes the `tools=` API path instead (production-current), but the text-parsing path remains the reference impl for local models that don't honor the `tool` role cleanly.
...
- **Connects to:** W5 Pattern Zoo (ReAct is one of 8 patterns); W5.5 Metacognition (Reflexion = ReAct + episodic scratchpad + self-critique); W7 Tool Harness (`run_tool()` and per-tool budget registry seed the production harness); W11.5 Agent Security (path-containment guards on `read_file`/`write_file`, and turning `python_repl`'s non-sandbox into a real one — env strip, setrlimit, container/bubblewrap); W3.5.5 Multi-Agent Shared Memory (the guild coordination layer is consumed by this agent loop — the loop is the per-agent runtime the shared memory coordinates across).
- **Foreshadows:** W6 Context Governance (FIFO eviction → proper autocompact with reserved 20K-token summary); W11 System Design (production ReAct deployment — observability, escalation ladders, stop conditions); W12 Capstone (the loop in `src/react.py` is the runtime substrate); W12.5 Multimodal Agents (the cross-modal retrieval agent reuses this chapter's ReAct reasoning loop over visual + textual evidence).
