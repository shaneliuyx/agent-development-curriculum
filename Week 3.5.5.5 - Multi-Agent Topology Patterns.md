...
**Result** *(measured 2026-05-28, post BCJ Entry 6 fix)*: `python code/hierarchical.py` on "Compare regulatory frameworks for AI across EU, US, and UK" against oMLX `gpt-oss-20b`. `total_wall_s = 115.46`, `plan_wall_s = 9.05`, `sub_walls_s = [58.90, 68.15]`, `max_sub_wall_s = 68.15`, `synthesize_wall_s = 38.25`, `agents_total = 7`. Observed total matches `plan + max(subs) + synth` (115.45s) within 0.01s rounding; sequential-equivalent would be 174.35s. **Speedup factor = 174.35 / 115.46 = 1.51×.** Less than supervisor's 1.77× (Phase 1) because hierarchical's sequential overhead (plan + synth = 47.30s) is 41% of total — Amdahl floor for 2-sub-lead parallelism with high serial fraction. TOP synthesize produced a 7-row EU/US/UK comparison table with explicit gap acknowledgments — the reasoning-model trap (empty `content`, CoT-loop on missing material) closed by the 3-layer fix in BCJ Entry 6. Honest caveat: gap-acknowledge prompt is **label-and-fill, not label-and-stop** — the model populated the UK column with its training-data knowledge despite the "do not speculate" instruction, while also flagging UK as a gap in the explicit gaps section. Acceptable trade-off vs the empty-answer trap. ~7× token cost vs single-agent (1 top-plan + 2 sub-plans + 4 leaves + 2 sub-synths + 1 top-synth = 10 LLM calls). Hierarchy WINS when question has genuine 2-layer decomposition (regulatory comparison across regions confirmed this — sub-leads naturally split EU vs US); LOSES (vs flat supervisor with 6 workers) when question is one-layer. See `lab-03-5-5-5-topology/RESULTS.md` §"Phase 2 hierarchical — manual run measurement" for full breakdown.
