...
> _W0 has no runnable lab — these are architectural decisions, not measured findings. Each soundbite states the design rationale and points to the downstream lab (W1 / W2 / W4 / W2.5) where the actual numbers were measured. Pre-experiment fabricated numbers were removed in the 2026-05-07 audit pass._

**Soundbite 1 — Why local-first matters for an agent stack (architectural decision).** I committed to local-first inference on Apple Silicon before writing any lab, on two arguments: cloud latency floor (round-trip + queue) makes iterative ReAct loops slow even at sub-second model time, and per-request cost compounds when bad-case journaling demands repeated runs against the same prompt. The validation lives downstream — W2's reranker latency table (`lab-02-rerank-compress/RESULTS.md`) confirms BGE-reranker-v2-m3 lands at 37.8 ms / batch on MPS+fp16 (5.3× under the 200 ms budget), and W4's ReAct loop (when it lands) will measure the actual end-to-end tool-call cycle. The W0 soundbite is the *commitment*; the labs are the *receipts*.

**Soundbite 2 — Why BGE-M3 runs on PyTorch MPS rather than MLX (port-investigation result).** I investigated porting BGE-M3 to MLX in W0 and decided against it on two arguments: BERT-style encoders do one forward pass per batch and gain little from MLX's fused autoregressive kernels (which target token-by-token decode), and 4-bit quantization — the technique that makes LLMs viable on 48 GB unified memory — degrades embedding geometry because the output vector IS the similarity target, not a 50k-way argmax that tolerates logit noise. The W2 reranker port investigation (`lab-02-rerank-compress/RESULTS.md` §4) reached the same conclusion for cross-encoders: `mlx-lm` rejected `xlm-roberta` as encoder-only with a missing classifier head; defer hand-port until QPS justifies. The decision pattern — fp32 on MPS for embedding-style models — survived three separate labs.

**Soundbite 3 — Three-tier model routing as a cost-control primitive (validated in W2.5).** I mapped the oMLX model roster to haiku/sonnet/opus tiers in W0: gpt-oss-20B for fast single-hop retrieval checks, Gemma 26B for standard reasoning, Qwen 35B for architecture work. Every script selects a tier by name, not by model ID, so a backend swap never touches application code. W2.5 validated this in production: `MODEL_HAIKU=gpt-oss-20b-MXFP4-Q8` was originally used for entity extraction and produced non-deterministic empty outputs (Bad-Case Entry 6 — reasoning-model token-budget exhaustion); switching extraction to `MODEL_SONNET=gemma-4-26B` was a one-line config change. The tier abstraction made the fix possible without refactoring extraction code. Cloud APIs are deferred until Weeks 7–8 where the curriculum explicitly needs them; total cloud spend will be measured against the W7/W8 budget when those labs run, not predicted in W0.
