...
async def test_quest_lifecycle_round_trip() -> None:
    ...
    All assertions are substring-match against text responses — guild's MCP
    surface returns human-readable text, not structured JSON (see class
    docstring)."""
    async with GuildClient(agent_id="alice") as gc:
        # 1. Post a quest. Capture the server-assigned QUEST_ID
        #    (regex-extracted from the response text 'posted QUEST-N:').
        ...
        assert quest_id.startswith("QUEST-"), f"unexpected response: {quest_id!r}"

        # 2. Atomically claim. Response text contains 'accepted' on success.
        accept_resp = await gc.quest_accept(quest_id)
        assert "accept" in accept_resp.lower() or "claim" in accept_resp.lower(), \
        ...
        # 4. Fulfill with the required report. Response contains 'fulfilled'.
...
- **Park, Joon Sung et al. (2023). Generative Agents — Interactive Simulacra of Human Behavior.** arXiv:2304.03442. The "Smallville" experiment — 25 generative agents in a sandbox town with daily routines, memory streams, reflection, and emergent social behavior (party planning, gossip propagation). Foundational reference for **multi-agent simulation** beyond task-completion: agents with explicit memory + reflection + plan layers produce emergent social phenomena. When interview-asked "what's the upper bound on multi-agent emergence?", Smallville is the canonical answer. Distinguish from production multi-agent systems (this chapter's focus): Smallville agents are LONG-LIVED + REFLECTIVE; production agents are typically TASK-BOUNDED + STATELESS. The memory architecture (importance-weighted retrieval + nightly reflection) is the part most often lifted into production.
- **MAST (Multi-Agent System Taxonomy) — failure modes framework.** Phase 16 lesson 23 in `rohitg00/ai-engineering-from-scratch`. Catalogs production multi-agent failure modes — **groupthink** (agents converge on wrong answer because they read each other's earlier wrong outputs), **cascade failures** (one agent's bad output corrupts all downstream agents), **deadlock** (agents waiting for each other's outputs in a cycle), **role drift** (specialized agent gradually broadens into a generalist + loses specialization benefit), **silent disagreement** (multi-agent synthesis picks one answer without surfacing that agents disagreed). Defense patterns: explicit disagreement protocols + adversarial-role assignment + timeout-based termination + role-boundary audits. Treat MAST as the OWASP-equivalent catalog for multi-agent system threats — every production deployment should run an MAST-style audit before launch.
