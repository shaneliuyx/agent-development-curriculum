...
**Soundbite 1 — "When does tree-index RAG beat vector and graph on a long structured document?"**

"Tree-index wins when the document's own table of contents is the natural retrieval primitive — section identity matters more than surface similarity. On my Berkshire 10-K three-way comparison, tree clearly outperformed both other backends on citation-required and cross-section synthesis questions, while graph degraded badly on a single-document corpus because its entity graph collapses to a degenerate star. The win only happened once I replaced greedy descent with a full agentic loop: the model issues fetch-page-range tool calls, observes content, and re-decides — rather than committing to one branch at the root."

*Principle:* tree-index retrieval is downstream of document structure, not query similarity — and the loop matters as much as the index.

**Soundbite 2 — "What's the failure mode of tree-index retrieval, and how do you fix it?"**

"Two structural blind spots. First, the navigator only sees section titles and page ranges — body text never enters its context, so purely factual lookups that depend on body content go to zero. Vector handles those because it treats body text as primary. Second, greedy descent commits to a single branch, so queries spanning two sub-trees retrieve one and miss the other. The fix isn't beam-search at the tree level — it's routing: a lightweight classifier sends cross-section synthesis to a cluster pre-fetch layer that locates the right page ranges before the first LLM call, rather than discovering them through multi-iteration descent."

*Principle:* tree-index failures are structural, not tunable — route around the blind spots rather than patching the navigator.

**Soundbite 3 — "Why run three retrieval lanes instead of a single universal pipeline?"**

"Because each lane has a different cost shape and a different fit zone, and a universal pipeline forces you to pay all three costs on every query. Vector is cheap to ingest and query, wins on paraphrase and factoid. Graph is expensive to build, suited to multi-hop relational questions, but degenerates on single-document corpora. Tree query is expensive per call, wins on long-document precision and citation traceability. The router is lightweight — a small classifier whose misroute cost is bounded, since the wrong lane returns a worse answer, not a fabricated one. That asymmetry is what makes routing the economically correct architecture when the query mix is heterogeneous."

*Principle:* architecture is downstream of query type and cost tolerance — the router is the cheapest component and the highest-leverage one.
...
- **Zheng et al. (2023).** *Judging LLM-as-a-Judge with MT-Bench and Chatbot Arena.* NeurIPS 2023 D&B. arXiv:2306.05685. The canonical paper on LLM-judge bias types: **position bias** (judge prefers first option), **verbosity bias** (judge prefers longer answers), **self-enhancement bias** (judge prefers outputs from its own model family). Justifies the GT-judge methodology adopted in Phase 9 — fixed reference answer pinned per question kills all three biases simultaneously, which is why GT-judge moved Tree-v3 from 0.885 (vanilla judge) → 1.000 without changing the retriever at all.
- **Husain, Hamel (2024).** *Creating a LLM-as-a-Judge That Drives Business Results.* hamel.dev. The practitioner blog on writing judge prompts that catch real defects rather than rephrasing the rubric. Directly applicable to the §4 GT-judge prompt evolution — error-analysis-first iteration over judge prompts, not just retriever prompts.
- **Kwon et al. (2023).** *PagedAttention / vLLM.* SOSP 2023. arXiv:2309.06180. Relevant context for BCJ #23 (long-context budget exhaustion in the agentic loop): vLLM-style paged KV-cache + prefix caching is the production analogue of what oMLX serves; LMCache (2025) extends this with prefill-decode disaggregation that becomes load-bearing when cluster-summary prefills dominate per-iteration cost.
