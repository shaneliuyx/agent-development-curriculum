...
> **Status: IMPLEMENTED + MEASURED (lab run 2026-06-16).** All six Exit-Criteria artifacts are built and measured on the M5 Pro / oMLX `:8000` fleet (`lab-w45-routing/RESULTS.md`); measured numbers are wired through every Phase *Result* table and the Phase 5 four-way bench. Headlines (each traced to a §5 entry): 2-way tier **95.65%** / mode **91.30%** on the 23-row eval — the 3-way tier plateaued at the **82.61%** label-noise ceiling, so the taxonomy was collapsed sonnet+opus→heavy (Entry 2); the BART-MNLI vote *regressed* tier to **60.87%** and was dropped (Entry 1); the Phase 5 cost-latency bench runs in **7m49s** after batch-by-resident-model (Entry 5) and shows routing is **Pareto-dominated** on this hard-skewed workload (Entry 6); the optional Phase 6 privacy axis measures a **0/8 leak rate** with a 1/4 over-restriction rate (Entry 7). §5 holds **7 measured entries** (no pre-flight predictions), now **synced to the global `Bad-Case Journal.md`** (Pattern 21). §6 Soundbites and §7 References are user-voiced and cite the measured arc. Spec source: cross-repo convergence research on Personal_AI_Infrastructure / PraisonAI / AutoGPT / agenticSeek (2026-05-14); enriched with PilotDeck / ClawXRouter (2026-06-16).
>
...
- [x] `src/router.py` — local Qwen3.5-4B classifier emitting `{tier, mode}` from prompt + last-N scratchpad turns *(measured; 2-way `src/router2.py` after the taxonomy collapse — Entry 2)*
- [x] `src/tier_dispatch.py` — swaps the `model:` field on the single oMLX `:8000` endpoint per classifier verdict; deterministic fallback on cold-load / 507 memory ceiling
- [x] `src/router_vote.py` — second classifier (BART-MNLI) voting against the primary + disagreement logging *(built and measured — it regressed accuracy, so dropped as a mechanism and kept as a label-review queue; Entry 1)*
- [x] `tests/test_router_accuracy.py` — 60-prompt probe set; **measured 95.65% per-tier (2-way) ≥ 85%, 91.30% per-mode ≥ 90%** (3-way tier hit the 82.61% label-noise ceiling — Entry 2)
- [x] `RESULTS.md` four-way bench: opus-always / classifier-routed / classifier+vote / random — measured (Phase 5, 7m49s); routing is Pareto-dominated on this hard-skewed set (Entry 6)
- [x] *(optional stretch — Phase 6)* `src/privacy_router.py` — S1/S2/S3 edge-vs-cloud via two-phase rule+LLM detector; **measured 0/8 leak rate, 1/4 over-restriction** on the privacy probe set (Entry 7)

---

## 1. Why This Week Matters 

W4 built one ReAct loop calling one opus-tier model. Every task — "what's 2+2", "summarize this paragraph", "debug this Python script", "plan a multi-step deployment" — pays the same 27B-parameter latency cost. Production agent systems don't work this way. Every request that reaches Claude.ai, ChatGPT, Cursor passes through a routing layer that picks the smallest model competent for the request — small classifier upstream, large executor only when needed. Daniel Miessler's PAI ships a Sonnet-backed mode classifier that routes prompts across MINIMAL / NATIVE / ALGORITHM tiers; agenticSeek runs a two-stage local classifier (Adaptive + BART-MNLI voted) before any tool dispatch. **The senior-engineer signal is "I can describe my routing layer's accuracy curve and its cost-latency Pareto front"** — and the reader who can say "my Qwen3.5-4B classifier splits easy-vs-hard at 95.65%, and I measured whether routing actually pays vs a route-everything-heavy baseline — it didn't on my hard-skewed set, and I can show you exactly why" sounds far more credible than one who picked one model and called it done. This chapter builds that classifier, measures its accuracy, and tests the cost-latency claim honestly on the W4 probe set — including where it fails.
...
The first property is that **the router is a classifier, not a reasoning model**. It emits one of N labels: `{haiku, sonnet, opus} × {minimal, react, deliberate}` = nine cells, or a flatter taxonomy if you prefer. The router does not reason about the prompt; it pattern-matches it to a closed list. This is why a 4B-parameter model with a strong system prompt outperforms a 7B reasoning model with a free-text "decide what to do next" — calibration matters more than capability when the output space is bounded.
...
Goal: run the eval split of the probe set under four routing configurations and produce the cost-latency Pareto front. The comparison is the chapter's central measurement; it's how you defend a routing claim with a measured cost×success Pareto front instead of vibes — and here it showed routing Pareto-dominated on a hard-skewed workload, which is itself the result.
...
- **`RESULTS_phase5.json` is the artifact you cite in interviews.** The honest, measured pitch after Phase 4b: "A 2-tier `{haiku, heavy}` router classifies easy-vs-hard at 95.65% with a local 4B, routing the easy majority off the heavy model — buying most of heavy-tier quality at a fraction of its cost. I got there by *disproving* a fancier vote design and collapsing a taxonomy boundary that had only 78% inter-annotator agreement." That's measure → diagnose → reframe, not a vote that "works."
...
*Root cause:* BART-MNLI is an *independent but incompetent* voter — a topic classifier judging routing-meta labels disagrees with the Qwen classifier on **83%** of rows, and the "escalate on disagree" rule then bumped 83% of queries to the heavy tier (de-facto heavy-always). A confidence-gated vote and a learned `AdaptiveClassifier` second voter both avoided the regression but added **no gain** — a vote cannot beat its best voter.
*Fix:* drop the vote. (Lab `RESULTS.md` BCJ-2/BCJ-4.)

**Entry 2 — The tier accuracy ceiling was the LABELS, not the model.**
*Symptom:* 3-way tier accuracy plateaued at exactly **82.61%** across a 4B, gemma-26B, Sonnet, and Opus — no bigger model and no vote could beat it.
*Root cause:* a blind Opus re-label of the eval set agreed with the original tiers only **78%** (5/23 disputed, all on the sonnet↔opus boundary). A classifier cannot exceed the self-agreement of its ground truth, so 82.61% *is* the label-noise ceiling — a too-fine taxonomy, not a model limit.
*Fix:* collapse the contested boundary — merge sonnet+opus into one `heavy` tier. 2-way tier jumped to **95.65%** with the same 4B (`src/router2.py`). When an accuracy wall is flat across model sizes, suspect the labels. (BCJ-7.)

**Entry 3 — Frontier model answered the prompt instead of classifying it (VibeProxy persona-cloak).**
...
*Root cause:* VibeProxy (:8317) is a Claude-Code router that injects its OWN system prompt server-side; a caller-supplied `system` role triggers the persona-cloak (recurrence of the W3.5.9 finding). User-only callers survive; system-role callers do not.
*Fix:* no caller `system` role — fold the rubric into the first `user` turn. Parse failures 6 → 0; Sonnet/Opus then scored 82.61 tier / 91.30 mode. (BCJ-5.)

**Entry 4 — Reasoning model rejected a standard parameter (frontier-API drift).**
...
*Fix:* omit `temperature` for reasoning models; gate model-specific params by model id. (BCJ-6.)

**Entry 5 — The Phase 5 bench hung >1 h (cold-load thrash on one-hot oMLX).**
...
*Fix:* loop `for config:` and **sort each config's rows by executor model** so each model loads once per config. 1 h+ → **7m49s**. On one-hot hardware, batch by resident model. (BCJ-8.)

**Entry 6 — A "green" bench that measured nothing (saturated grader + starved executor).**
...
> **All figures below are measured** (2026-06-16 lab runs; full numbers in the lab `RESULTS.md`).

**(a) "How would you design a routing layer for an LLM agent system?"**
I build the router as a classifier, not a reasoning model — a Qwen3.5-4B emitting a closed-list `(tier, mode)` verdict. The honest measured arc: a 3-way `{haiku, sonnet, opus}` taxonomy plateaued at 82.61% tier across a 4B, gemma-26B, Sonnet, AND Opus — a blind Opus re-label of the eval set agreed with the labels only 78%, so the ceiling was inter-annotator label noise, not model capability. I collapsed the contested sonnet↔opus boundary to `{haiku, heavy}` and tier jumped to 95.65% with the same 4B. The load-bearing artifact isn't the dispatcher — it's the system prompt, the hand-labelled probe set, and a taxonomy granularity the labels can actually support.

**(b) "You added a second classifier to vote — did it help?"**
No — and I measured why. A single classifier mis-routes silently, so I added a BART-MNLI vote. It *regressed* tier from 82.61% to 60.87%: BART is independent but incompetent (a topic model that disagreed with the router on 83% of rows), and the escalate-on-disagree rule then fired on 83% of queries — de-facto route-everything-heavy. A confidence-gated vote and a learned `AdaptiveClassifier` second voter both avoided the regression but added no gain. The lesson: a vote can't beat its best voter — a second classifier helps only if it's independent AND individually accurate. I kept the disagreement log as a label-review queue and dropped the vote as an accuracy mechanism.
...
**(d) "What scheduling lesson did the routing benchmark teach you?"** *(Interview Q9 cover; see [[Interview Question Index]] Tier 4)*
That on one-hot local inference the dominant cost is model cold-loads, not request concurrency. My first cost-latency bench ran over an hour because a `for row: for config:` loop swapped the resident heavy model ~90 times (~10–30 s each). Reordering to loop per-config and sorting each config's rows by executor model — so each model loads once per config — cut it to 7m49s: same 69 calls, ~6 cold-loads instead of ~90. The win was scheduling by resident model, not parallelizing the calls.
...
4. **Vote:** ❌ tried (BART-MNLI + a learned `AdaptiveClassifier`), measured net-negative (BART regressed tier 82.61→60.87; the gated/learned votes added no gain) and **dropped** — the shipped router is the single 2-tier few-shot classifier (§5 Entry 1).
