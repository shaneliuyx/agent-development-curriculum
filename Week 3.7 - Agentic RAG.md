...
**Soundbite 5.** *(Comparison / multi-hop queries — §3.3.1.)* A single web search for "compare BNSF and Berkshire Energy 2023 revenue" abstained — no one page held both figures. So the web fallback fans out: decompose into atomic sub-queries, search each independently, round-robin interleave the results so neither entity gets truncated under the synthesizer's 8000-char passage cap, then synthesize over the union. That turned a total abstain into both figures cited — BNSF $23.876 billion, Berkshire Energy $26.198 billion, grounded from the web. Decomposition belongs on whichever surface holds the answer; here that's the web, not the corpus.
