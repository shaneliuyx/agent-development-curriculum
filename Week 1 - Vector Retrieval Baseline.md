...
**Soundbite 1 — Why BGE-M3 + what the W1 number actually means.** BGE-M3 is my default dense retriever — single forward pass produces a 1024-dim dense vector and a sparse lexical vector together, semantic generalization without sacrificing exact-token recall. On the Week 1 MS MARCO 10K slice with HNSW m=16/ef=128, BGE-M3 hit recall@10 = 0.9933 and MRR@10 = 0.9556. 99.3% of queries had the gold passage in their top-10 — that's saturation, not headroom for a reranker to push against. The lesson: a 10K MS MARCO slice is too easy to differentiate retrievers, which is exactly why W2 switches to BEIR-FiQA — ceiling-free at dense recall@100 ≈ 0.61.

**Soundbite 2 — The HNSW ablation null result.** I compared m=16/ef=128 against m=8/ef=64 on the same BGE-M3 vectors — recall, MRR, and nDCG came back **bit-identical** at full float precision (0.9933 / 0.9556 / 0.9637 across both configs, all 6,980 queries; verified against raw JSON, not rounded display). The only measurable difference was search wall time, 29.0s vs 27.2s — a ~6% speedup on the smaller graph. The actionable rule: don't bother tuning HNSW config below ~1M vectors — at 10K, both graphs are over-connected enough that greedy descent lands on the exact same top-10 for every query. The null result IS the data; document the threshold where the comparison would actually matter.

**Soundbite 3 — Saturation as the signal to switch corpora.** The W1 baseline exists so every downstream improvement has a denominator. With BGE-M3 + HNSW already at recall@10 = 0.9933 on MS MARCO 10K, the corpus is saturated — a cross-encoder reranker or hybrid RRF cannot move the needle here in any measurable way. That saturation IS the signal to switch corpora: W2 moves to BEIR-FiQA where dense recall@100 sits around 0.61, leaving ceiling-free room to measure the actual lift from cross-encoder rerank, hybrid fusion, and chunking variants. Without W1 establishing saturation, W2's corpus switch would lack a justification — the discipline is to publish your null result loud enough that the next experiment isn't designed against the wrong corpus.
