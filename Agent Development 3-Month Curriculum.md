...
The measurement + scaling cap of the W3.5 cluster. Prerequisite: W3.5.8 (two-tier guild + EverCore architecture) is shipped. W3.5.9 (a) integrates EverOS's `HyperMem` as the L3 relational tier (extending two-tier → three-tier when query mix calls for it), and (b) runs the industry-standard memory benchmark (LongMemEval `oracle` subset) across all five backends to produce comparable scores. Subsumes the retired W3.5.7's benchmark concern at full depth.

**Theory (1.5h).**
- Read: HyperMem README + algorithmic overview (hypergraph data model — typed edges across ≥2 entity nodes); [LongMemEval paper](https://arxiv.org/abs/2410.10813) on the 5 memory-task categories (single-session-user / multi-session-user / temporal-reasoning / knowledge-update / abstention); LoCoMo for the longer-conversation companion benchmark; EverMemBench README for the in-repo memory-quality eval shape.
- Master: when hypergraph beats flat semantic memory (multi-entity intersection queries — "experts on X AND Y AND Z together"); the three-tier graduation trigger (≥30% of queries are multi-entity-intersection); why benchmark-against-published-scores is the senior-signal eval discipline; the difference between MEASURING and OPTIMIZING-FOR a benchmark (cargo-cult vs Pareto-frontier navigation).

**Lab (5–6.5h) — `lab-03-5-9-bench-hypergraph`.**
1. **Phase 1 — HyperMem service (~30min)**: extend the W3.5.8 docker-compose to bring up HyperMem alongside EverCore. Smoke-test the HyperMem HTTP API (typed-edge create, hyperedge query, relational traversal).
2. **Phase 2 — Three-tier wrapper (~2h)**: extend `TieredMemory` from W3.5.8 → `ThreeTierMemory(guild, evercore, hypermem)`. Add `query_relations()` method that routes multi-entity-intersection queries to HyperMem and pass-through queries to EverCore.
3. **Phase 3 — Consolidation pipeline extension (~1.5h)**: extend the W3.5.8 consolidation batch job to ALSO extract typed edges from completed scrolls and imprint them into HyperMem. Edges have idempotency keys (scroll_id + entity-pair hash) so re-runs don't accumulate duplicates.
4. **Phase 4 — LongMemEval `oracle` subset run (~2-3h)**: download the LongMemEval `oracle` subset (~50 questions across 5 categories). Write a benchmark client that calls each backend (no-memory / guild-only / EverCore-only / two-tier / **three-tier**) on the SAME 50 questions. Capture per-category pass rate + mean latency + total tokens.
5. **Phase 5 — Analysis + RESULTS.md (~1h)**: 5-way comparison matrix. Identify which categories each tier wins. Cite EverCore's published LongMemEval score (83%) and locate where your three-tier sits on that scale. Document the falsifying-hypothesis discipline (cf. W3.5 Phase 5 mem0 cross-check) — propose hypothesis, design experiment, observe result, update belief.

**Exit criteria.** 90-second answer to "how do you scale a multi-agent memory system AND measure it against published baselines?" — name the three-tier pattern (operational + semantic + relational), the LongMemEval methodology + 5 categories, the 5-way comparison matrix differentials, AND the trigger condition for graduating from two-tier to three-tier (multi-entity-intersection query rate ≥ 30%). Cite the measured score gap between your three-tier system and EverCore's published 83% on the same `oracle` subset.
...
| 3.5.8 | [[Week 3.5.8 - Two-Tier Memory Architecture]] | Two-tier capstone + R2-expanded with 3 STRETCH Phases + bitemporal extension. Phase 1-2: spin up guild + EverCore; `TieredMemory` two-layer identity (agent_id + user_id). Phase 3 consolidation pipeline with idempotency SQLite + atomisation + quality-score promotion gate. Phase 4 two-agent shared-knowledge demo. Phase 5 four-way benchmark + LongMemEval `oracle`. **Phase 7 STRETCH** — Qdrant drop-in (one-line import swap; 4/4 tests; 20-30× faster imprints). **Phase 8 STRETCH** — EverCore Bucket-1 demo with measured 35× Qdrant speedup on imprint + retrieval-shape divergence. **Phase 9 STRETCH** — online dedup-and-synthesis (Batchelor-Manning form #1; `facts_deduplicated=2` measured). **Phase 9.6 STRETCH** — bitemporal supersede/coexist 6-action classifier (5/5 dedup-suite tests PASS in 76.5s). 15 BCJ entries (6-15 observed). |
| 3.5.9 | [[Week 3.5.9 - Memory Benchmarks and Hypergraph Three-Tier]] | Measurement + scaling cap. Prerequisite: W3.5.8 shipped. Phase 1 bring up HyperMem service alongside EverCore via docker-compose. Phase 2 extend `TieredMemory` → `ThreeTierMemory` with `query_relations()` routing multi-entity-intersection queries to HyperMem. Phase 3 extend consolidation pipeline to ALSO extract typed edges → HyperMem (idempotent by scroll_id + entity-pair hash). Phase 4 LongMemEval `oracle` subset run across 5 backends (no-mem / guild / EverCore / two-tier / **three-tier**). Phase 5 5-way comparison matrix with per-category breakdown + EverCore's published 83% as the anchor. |
