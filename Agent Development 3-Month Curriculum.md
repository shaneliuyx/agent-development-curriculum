...
**Career-track overlay.** [[FDE Track - Forward Deployed Engineer Path]] is an overlay note — not a week — that re-threads the full curriculum into the **Forward Deployed Engineer** path: it maps every week onto the FDE skill stack (agentic orchestration, evals and guardrails, MCP integration, production deployment, customer context), gives a compressed FDE reading spine, and names the one skill the curriculum cannot teach by default — customer context. Read it early if FDE is your target role. The customer-context gap it identifies is closed by **Week 12's FDE Delivery Mode** (a process overlay on the capstone: stakeholder intake → scoped SLA → handoff README → one feedback cycle), which produces a `delivery/` directory in the capstone repo as the evidence of the role's consulting half.
...
- **Cross-session memory benchmark** — 15/15 recall + Phase 5 mem0 cross-check 10/14 (W3.5), 5-backend LongMemEval slice comparison via requirement-derived hybrid router (W3.5.9 — methodology + design shipped, Phase 3-5 measurements pending).
...
- **The full memory cluster** (W3.5 → W3.5.5 → W3.5.8 → W3.5.9 → W3.5.95): cross-session memory (15/15 recall + mem0 cross-check 10/14), multi-agent shared memory via guild MCP atomic-claim (W3.5.5), **two-tier architecture + Phase 6 Qdrant drop-in + Phase 7 EverCore Bucket-1 + Phase 8 online dedup-and-synthesis + Phase 8.6 bitemporal supersede/coexist** (W3.5.8 — 5/5 dedup-suite tests PASS in 76.5s + 35× Qdrant speedup measured + 15 BCJ entries), requirement-driven memory architecture decision framework + homebrew hybrid router (W3.5.9 — design shipped, Phase 3-5 measurements pending), self-observability memory primer (W3.5.95).
...
### Week 0.3 — Agent History + Foundational Narrative (~4h, reading + writing only)
> Detailed runbook: [[Week 0.3 - Agent History and Foundational Narrative]]

**Why this week exists (added 2026-05-19).** Many interviews open with "tell me about your understanding of agents" — a softball question that exposes whether the candidate has DEPTH or just buzzword fluency. Candidates who trace the lineage from symbolic AI through reactive robotics through BDI through reinforcement learning to LLM agents sound principled, not tactical. Cross-references hello-agents Ch 1+2 (Datawhale's Chinese-language narrative).

**Exit criteria.** Articulate 5 agent eras + 1 representative system per era (SHRDLU / Roomba / JADE / AlphaGo / AutoGPT). Trace 2 lineage points (Era 3 BDI → MCP/A2A vocab; Era 4 RL → GRPO/RLHF). Answer "what's an agent vs chatbot vs function call?" via the 3-property definition (goal-directedness / tool-use / state-driven decision-making). Write a 500-word essay + 1-slide reference + 3 interview soundbites for "tell me about your understanding of agents".
...
> Detailed runbook: [[Week 3.5.8 - Two-Tier Memory Architecture]] *(R2-expanded, Phases 6/7/8/8.6 IMPLEMENTED 2026-05-14/15)*

The capstone of the W3.5 cluster. W3.5 built single-agent memory; W3.5.5 added multi-agent coordination via guild; W3.5.8 wires guild (operational tier) and EverCore (semantic tier) into a single production-shaped two-tier architecture, then EXTENDS with three stretch Phases (Qdrant drop-in, EverCore Bucket-1, online dedup-and-synthesis) and a bitemporal supersede/coexist extension (Phase 8.6). 15 BCJ entries (6–15 observed against live lab runs).
...
6. **Phase 6 STRETCH — Qdrant drop-in (~2h, IMPLEMENTED commit `5e9bc69`)**: drop-in `TieredMemory` variant with one-line import swap; **4/4 tests pass**. Measured 20-30× faster imprints + 3-5× faster searches vs EverCore on Bucket-2 data.
7. **Phase 7 STRETCH — EverCore Bucket-1 demo (~3h, IMPLEMENTED 2026-05-15)**: 3 simulated dialogues (Alice / Bob / Carol) prove EverCore EARNS its cost on Bucket-1 conversation shape. Side-by-side compare yields **~35× Qdrant speedup on imprint** but EverCore wins on synthesised-episode retrieval shape. 3/3 episodes, 2/3 profiles measured.
8. **Phase 8 STRETCH — Online dedup-and-synthesis (~4h, IMPLEMENTED commit `bf1d091`)**: Batchelor-Manning form #1 — top-k semantic candidates + LLM-classified action (add/update/delete/no-op) at write time. 4-action classifier; `facts_deduplicated=2` measured on duplicate-scroll test.
...
### Week 3.5.9 — Requirement-Driven Memory Architecture + Three-Tier Hypergraph (intensive insert, ~22h, design+code shipped / measurements TBD)
> Detailed runbook: [[Week 3.5.9 - Requirement-Driven Memory Architecture]] *(filename retained for link stability; chapter scope now covers BOTH the requirement-driven decision framework AND a HyperMem-based three-tier implementation as worked example. Phase 1-2 analysis complete; Phase 3-9 code/design shipped; measurements TBD pending Phase 3-9 runs.)*

The meta-skill cap of the W3.5 cluster — now with a three-tier implementation as the worked example. W3.5 / W3.5.5 / W3.5.8 taught individual memory architectures (single-agent dual-store, multi-agent shared queue, two-tier consolidation). W3.5.9 teaches the META-SKILL — given a real published benchmark (LongMemEval), analyse the data shape, derive required memory primitives per axis, evaluate three candidate architecture classes (1-tier atomic-fact, 2-tier consolidation, graph-tier temporal), pick the architecture whose write-time primitive matches the read-time question shape, implement, verify. The chapter then implements the graph-tier extension end-to-end (HyperMem L3 over W3.5.8's L1+L2) — turning what was a "future decimal TBD" into a worked architectural example. Reader walks away with TWO deliverables: a defensible decision-making framework (Phase 1-2 + §2 class-capability matrix) AND a production-shaped three-tier hypergraph implementation (Phase 6-9, ThreeTierMemory wrapper + extended consolidation pipeline + 6-backend LongMemEval matrix).

**Theory (1.5h).**
- Read: [Mem0](https://arxiv.org/abs/2504.19413) (Wu et al. 2025 — atomic-fact 1-tier reference; claims 94.4 on full LongMemEval); [Graphiti / Zep](https://arxiv.org/abs/2501.13956) (Rasmussen et al. 2025 — graph-tier temporal reference); [MemGPT / Letta](https://arxiv.org/abs/2310.08560) (closest production parallel to W3.5.8's 2-tier pattern); the [LongMemEval paper](https://arxiv.org/abs/2410.10813) for the 6 question_type axes + cleaned dataset at `xiaowu0162/longmemeval-cleaned`. Batchelor-Manning 2026 survey continues to apply (the 6 write-time investment forms + 8-paradigm taxonomy).
- Master: requirement-derived architecture choice (NOT architecture-first thinking); the three architecture classes' write-time primitive trade-offs (1-tier wins atomic-fact recall, 2-tier wins narrative+profile+bitemporal, graph-tier wins temporal-edge queries); the joint-matrix decision rule (when single-class suffices vs when hybrid is justified — the 3/1/1-split heuristic); router patterns (question-type classifier vs confidence-fallback vs parallel-ensemble); the single-write + smart-read pattern as the senior-engineering alternative to dual-write reconciliation.

**Lab (~20h, Phase 1-2 analysis complete + Phase 3-9 code/design shipped / measurements TBD) — reuses `lab-03-5-8-two-tier` (no separate lab repo; this chapter extends W3.5.8's eval harness with new backends + HyperMem L3).**
1. **Phase 1 — Requirement Analysis (~1h, ANALYSIS COMPLETE)**: inspect actual LongMemEval samples. Decompose each `question_type` into a requirement vector along 5 primitive dimensions (atomic-fact / episode / profile / bitemporal / cross-session). Pure analysis — no code runs. Output: 7-row requirement matrix with per-axis justification + worked examples from real slice samples.
2. **Phase 2 — Architecture Decision (~1h, ANALYSIS COMPLETE)**: apply Phase 1's matrix against the §2 class-capability matrix. Derive per-axis winner. Joint result: 1-tier wins 3/7 axes, 2-tier wins 1/7, graph-tier wins 1/7, 2/7 are ties. 3/1/1 split → HYBRID JUSTIFIED. Architecture choice: 2-backend hybrid with question-type routing (graph-tier graduation rule documented: ≥30% multi-entity-intersection rate triggers L3 add).
3. **Phase 3 STRETCH — Open-source baseline: Mem0 (~2h, CODE SHIPPED + RESULTS TBD)**: `src/mem0_backend_adapter.py` (~80 LOC) wraps `mem0ai` SDK in TieredMemory-compatible interface, configured to share lab's local Qdrant + oMLX endpoint (apples-to-apples — same embed + same LLM as homebrew backends). `--backend mem0` dispatch added to eval driver. Measurement awaiting actual run.
4. **Phase 4 STRETCH — Homebrew hybrid router (~3h, CODE SHIPPED + RESULTS TBD)**: the chapter's first intellectual contribution. `src/atomic_fact_memory.py` (~120 LOC) per-message atomic-fact extraction → Qdrant (one LLM call per message emitting JSON array; one Qdrant point per fact). `src/router_memory.py` (~150 LOC) question-type classifier + dispatch (atomic_fact for IE + multi-session + temporal; tiered_2tier for knowledge-update). Single-write path + smart read routing (deliberately avoids dual-write reconciliation risk).
5. **Phase 5 STRETCH — Compare + Aggregate (~1h, METHODOLOGY DOCUMENTED + RESULTS TBD)**: extend `scripts/aggregate_results.py` for N-backend × 7-axis × N-question matrix. Sequential not parallel (resource-contention discipline). Axis-wise not aggregate-only (exposes which question types each backend handles). 2 cells pre-populated from W3.5.8 §7.7 measured baseline (qdrant 0/20 + evercore 0/20 on multi-session + knowledge-update); the remaining cells fill in after Phase 3-9 runs.
6. **Phase 6 — HyperMem Service Setup (~1h, DESIGN SHIPPED + DEPLOYMENT TBD)**: bring up HyperMem L3 via docker-compose at `:1996` alongside existing L2 EverCore `:1995`. Healthcheck loop, schema migration ordering against Postgres, port-collision recovery. Image build from EverOS source if not pre-built.
7. **Phase 7 — ThreeTierMemory Wrapper (~2h, CODE DESIGN SHIPPED + RUNS TBD)**: `src/three_tier_memory.py` (~200 LOC) extends W3.5.8's TieredMemory with L3 routing. New methods: `query_relations(entities)` for multi-entity-intersection queries; envelope translation between HyperMem response shape and lab-standard `content/score/metadata` envelope; question-type routing extended (temporal-reasoning + multi-entity-intersection → L3, all other axes → existing L1+L2).
8. **Phase 8 — Extended Consolidation Pipeline (~3h, CODE DESIGN SHIPPED + RUNS TBD)**: extend W3.5.8's `scripts/consolidate_scrolls.py` to ALSO emit HyperMem hyperedges (not just EverCore imprints). Closed-entity-enum schema enforced post-parse to prevent type drift. Provenance grounding (each edge cites source scroll). Idempotency via canonicalized entity-pair hash (handles `(A,B)` ≡ `(B,A)` collision).
9. **Phase 9 — Six-Backend LongMemEval Run + Analysis (~6h, METHODOLOGY DOCUMENTED + RESULTS TBD)**: 6-way comparison matrix (qdrant / evercore / mem0 / atomic_fact / hybrid_router / three_tier) × 7 axes × N questions. The chapter's primary measurement deliverable. Predicted L3 win on temporal-reasoning axis; honest null-result documentation if predicted win doesn't materialize. Graduation trigger empirically tested against the ≥30% heuristic.

**Exit criteria.** 90-second answer to *"how do you decide between 1-tier, 2-tier, and three-tier memory?"* — name the requirement-matrix decomposition, the joint-matrix decision rule, the 3/1/1-split-hybrid-justification, the ≥30% multi-entity-intersection graduation trigger, your measured per-axis scores including the temporal-reasoning delta between hybrid and three-tier (TBD pending Phase 3-9 runs). Bonus: when would you NOT build a router-based hybrid? (answer: when a single class wins ≥6/7 axes — pick the simpler operational shape). When would you NOT graduate to three-tier? (answer: when multi-entity-intersection query rate falls below ~30% — L3's operational cost isn't earned). Twin intellectual claims: (a) 80% of memory value comes from the WRITE-TIME PRIMITIVE; (b) architecture tier-count is downstream of workload measurement, not vice versa.

**Infra bridge.** The decision matrix is the SAME shape as a database engineer's "OLTP vs OLAP vs HTAP" decision — read-time access pattern determines write-time storage choice. The single-write + smart-read pattern is the production stand-in for synchronous dual-write — avoids reconciliation contracts (cf. database CDC-vs-dual-write trade-off). The question-type-router-by-explicit-label pattern is the same shape as a load-balancer's host-header-based routing — explicit metadata beats inferred classification at production reliability. The three-tier graduation rule (don't add a tier until measurement shows it's earned) mirrors the production-architect's habit of "describe what you're NOT building today + why" alongside what you ARE building.
...
### Week 6.4 — Low-Code Agent Platforms (Dify / Coze / n8n / LangFlow) (half-week insert, ~6h)
> Detailed runbook: [[Week 6.4 - Low-Code Agent Platforms]]

**Why this week exists (added 2026-05-19).** Most enterprise / product / consulting agent JDs assume candidates know at least one low-code platform (Dify, Coze, n8n, LangFlow, FlowiseAI, OpenWebUI, Copilot Studio, Bedrock Agents). The pure-code curriculum (W4 ReAct, W10 Framework Shootout) teaches you to BUILD; W6.4 teaches you to USE the platforms 80% of enterprise deployments actually run on. A candidate who can hand-roll a 150-LOC ReAct loop but can't talk Dify sounds out-of-touch when the interviewer asks "how would you ship this to a non-technical team?" Cross-references hello-agents Ch 5 (Datawhale's Chinese-language canonical text on low-code platforms).

**Lab (5h) — `lab-06-4-low-code`.** 4 phases: (1) self-host Dify + re-implement the W4 ReAct equivalent (4 tools + max-iter + memory + RAG knowledge base) via UI — measure time-to-ship vs W4's 150 LOC; (2) Coze plugin authoring — distinguishing primitive is multi-modal first-class; (3) n8n workflow automation — distinguishing primitive is trigger-based scheduling; (4) 5-axis low-code-vs-custom comparison matrix (audience / iteration speed / deployment surface / compliance / cost ceiling).

**Exit criteria.** 5-axis decision matrix written for "when low-code wins vs custom code." Articulate the 4 production failure modes specific to low-code (workflow canvas drift / token cost explosion / tool ABI drift / multi-platform fragmentation). 3 soundbites positioning low-code-platform fluency as a senior signal.

**Infra bridge.** Low-code platforms are the Zapier-IFTTT-for-agents layer. Same operational shape as any SaaS-automation platform — workflow canvas + tool integrations + deployment surface + audit log — with LLM primitives. Production agents typically use BOTH low-code (80% of standard workflows) + custom code (20% bespoke needs called via API).
...
### Week 9.3 — Agent Performance Evaluation (half-week insert, ~6h)
> Detailed runbook: [[Week 9.3 - Agent Performance Evaluation]]

**Why this week exists (added 2026-05-19).** W3 (RAG eval) + W11.6 (production tracing) cover retrieval-quality + cost-attribution. NEITHER covers what most agent interviewers actually ask: "how do you measure whether your agent is GOOD AT ITS JOB?" Agent eval is its own discipline — task completion / tool-selection accuracy / multi-step trajectory quality / cost-per-task — and the published benchmarks (AgentBench, GAIA, SWE-bench Verified, WebArena, OSWorld, τ-bench, MLE-bench) are how the industry actually compares agent systems. Cross-references hello-agents Ch 12 (Datawhale's *Agent Performance Evaluation* chapter).

**Lab (5h) — `lab-09-3-agent-eval`.** 3 phases: (1) run W4 ReAct agent against AgentBench-mini (subset like AgentBench-OS ~70 tasks); report pass rate against published GPT-4 baseline; (2) per-trajectory 5-dim rubric (terminated correctly / tool-selection / arg-correctness / no-hallucination / cost reasonable) + LLM-as-judge harness with Cohen's κ ≥ 0.6 gate against 20-50 hand-labeled gold trajectories; (3) Pareto plot pass-rate vs cost-per-task vs published baselines.

**Exit criteria.** Distinguish LLM eval vs RAG eval vs AGENT eval. Run W4 against one industry benchmark (AgentBench / GAIA / SWE-bench / WebArena) + report your pass rate honestly against published baselines (local-MLX ~weaker-than-frontier is expected). Identify the 4 agent-eval anti-patterns (golden-trajectory rigidity / single-metric optimization / single-benchmark overfit / closed-loop evaluator). LLM-as-judge protocol: judge ≠ agent model; require κ ≥ 0.6 before deploying.
...
|---|---|---|
| 0.3 | [[Week 0.3 - Agent History and Foundational Narrative]] | **Foundations narrative chapter (reading + writing only, ~4h).** 5 agent eras with 1 representative system per era: Symbolic AI (SHRDLU / SOAR / CYC, 1956-1990s) → Reactive/Behavior-based (Brooks subsumption / Roomba, 1986-2000s) → BDI + Multi-Agent (JADE / PRS / FIPA-ACL, 1995-2010s) → Deep RL (DQN / AlphaGo / AlphaStar, 2013-2020) → LLM Agents (AutoGPT / ReAct / Claude Code / Devin / o3, 2022-present). 2 explicit lineage points: Era 3 BDI vocab → MCP/A2A protocols (W6.8); Era 4 RL → GRPO/RLHF in W9.5. 3-property agent definition (goal-directedness / tool-use / state-driven decision-making). Required reading: Russell & Norvig Ch 2+25, Brooks 1986, ReAct paper, hello-agents Ch 1+2. Output: 500-word essay + 1-slide reference + 3 interview soundbites for "tell me about your understanding of agents". |
| 0.5 | [[Week 0.5 - LLM Internals Speedrun]] | **Foundations primer (interview Q13/Q14 cover).** 5-stage forward pass (tokenize → embed → positional → blocks → sample). Self-attention formula in LaTeX: $\text{Attention}(Q,K,V) = \text{softmax}(QK^\top/\sqrt{d_k})V$. Phase 1 tokenizer-comparison lab across Qwen3 / Gemma-3 / gpt-oss showing same-string-different-token-count. Phase 2 paper-and-pencil 4-token QKV worked example (row-0 weights $(0.39, 0.14, 0.235, 0.235)$). Phase 3 MLX same-token-different-position cosine demo (expected ~0.85-0.95). 3 interview soundbites; 3 pre-scoped BCJ entries. |
| 1 | [[Week 1 - Vector Retrieval Baseline]] | BGE-M3 dense + Qdrant collection setup (`02_collections.py`); corpus prep (`01_prep_corpus.py`); 3 ingestion variants (Nomic baseline + BGE-M3 standard + BGE-M3 fast); eval harness (`04_eval.py`) producing recall@k + precision@k + MRR on a 50-Q dev set; `model_config.py` env+model parameter discipline. Establishes the BGE-M3 baseline downstream labs (W2/W2.5/W2.7/W3) consume via `shared/rag_hybrid`. |
| 2 | [[Week 2 - Rerank and Context Compression]] | Phase 1 hybrid retrieval with Qdrant named vectors + RRF (Cormack 2009, k=60); FiQA harder benchmark breaking the dense-only ceiling. Phase 2 cross-encoder rerank (BGE-Reranker-v2-M3) + latency tuning (fp16 + batch=128 → 37.8ms per query, 5.3× under p95 budget); MLX-backed reranker variant. Phase 2.6 production wrapper `src/retrieve.py` (108 LOC, NATIVE_RRF Qdrant server-side fusion) consumed by W2.5/W2.7/W3/W3.7. Phase 3 context compression (`02_compress.py`). Phase 4 chunking sweep (9-cell grid). |
| 2.5 | [[Week 2.5 - GraphRAG]] | Neo4j + Wikipedia subset; entity/relation extraction with local Gemma; 2-hop subgraph traversal; 25-Q multi-hop head-to-head vs Week 2 vector RAG; comparison matrix in `RESULTS.md`; v12 Wikidata QID linking closed entity-fragmentation ceiling. v12.4m universal read-path overhaul: ALL judge 0.96, factoid 1.00, multi_hop 0.88, **W/L/T 32/0/0 vs VectorRAG** (entity-recall judge); the May 7 result was reversed in W2.7's GT-judge re-comparison (graph 0.375 — graph DOES degenerate on single-document corpora once entity-recall stops favoring it). |
| 2.7 | [[Week 2.7 - Structure-Aware RAG]] | PageIndex / tree-index RAG on Berkshire 2023 annual report (152 pages); 4-index architecture (LLM tree + K-means cluster + entity reverse-index + BGE-M3 hybrid page-vector fallback); agentic multi-iter loop with cluster pre-fetch + BUDGET EXHAUSTED 5-rule synthesis + chunk-level fallback; same-corpus three-way comparison vs vector + graph; per-Python-block bundle structure (mermaid → code → walkthrough → result → insight) — **canonical reference for the W2.7+ per-block-bundle pattern**; GT-judge methodology (binary pass/fail against PDF-grounded `pass_criteria`) replaces entity-recall; Phase 9 ceiling = **16/16 = 1.000** (vector 0.500, graph 0.375). |
| 3 | [[Week 3 - RAG Evaluation]] | Phase 1 dev set generation (`01_gen_dev_set.py`, 50 questions from corpus). Phase 2 RAGAS eval harness (faithfulness + context-precision + answer-relevancy); 2-row baseline. Phase 3 HyDE A/B vs Phase-2 baseline. Phase 4 multi-query fusion A/B; pre-migration vs current pipeline variants. Phase 5 Phoenix traces wired end-to-end. ADR template for retrieval-architecture decisions. Two-tier eval discipline: RAGAS for commit-level regression detection + GT-judge (W2.7) for release-gate runs. |
| 3.5 | [[Week 3.5 - Cross-Session Memory]] | mem0 + Qdrant + SQLite dual-store; recall/remember REPL (`chat.py`); 3-session demo proving cross-session recall (`demo_three_sessions.py`); `lab_init.py` guided setup mirroring `guild init` DX; 15-Q recall benchmark with contradiction-update + multi-fact composition tests (15/15 passing); **Phase 5 mem0 cross-check** wrapper (`memory_mem0.py`, ~132 LOC, 10/14 passing) demonstrating hand-rolled-vs-library architectural differences. |
| 3.5.5 | [[Week 3.5.5 - Multi-Agent Shared Memory]] | Install `mathomhaus/guild` (Go MCP server, single binary, embedded SQLite, BM25+dense+RRF retrieval); two parallel MCP clients (Claude Code + Cursor) sharing one substrate; atomic-claim scenario proving exactly-one-winner lock; 3-act cross-session handoff (A → B → C scroll chain); side-by-side W3.5-vs-guild comparison matrix; 15-Q multi-agent recall benchmark (same-agent + cross-agent + contradiction-during-parallel-work). Chapter fully written 2026-05-12 + lab commit `e5b91e9`. |
| 3.5.5.5 | [[Week 3.5.5.5 - Multi-Agent Topology Patterns]] | **SPEC v0 — topology axis for multi-agent systems.** Five canonical topologies — supervisor / hierarchical / group-chat speaker-selection / handoffs-routines / voting-debate. Anthropic Research production 90.2% improvement vs single-agent + "80% of variance from token usage alone" finding as the load-bearing rationale for supervisor. OpenAI Swarm two-primitive API (routine + handoff). AutoGen / AG2 / Microsoft Agent Framework GroupChat history. 6-question decision matrix for topology choice + 6 lab phases SPEC. Source: Phase 16 lessons 05/06/10/11/15 (`rohitg00/ai-engineering-from-scratch`). |
| 3.5.8 | [[Week 3.5.8 - Two-Tier Memory Architecture]] | Two-tier capstone + R2-expanded with 3 STRETCH Phases + bitemporal extension. Phase 1-2: spin up guild + EverCore; `TieredMemory` two-layer identity (agent_id + user_id). Phase 3 consolidation pipeline with idempotency SQLite + atomisation + quality-score promotion gate. Phase 4 two-agent shared-knowledge demo. Phase 5 four-way benchmark + LongMemEval `oracle`. **Phase 6 STRETCH** — Qdrant drop-in (one-line import swap; 4/4 tests; 20-30× faster imprints). **Phase 7 STRETCH** — EverCore Bucket-1 demo with measured 35× Qdrant speedup on imprint + retrieval-shape divergence. **Phase 8 STRETCH** — online dedup-and-synthesis (Batchelor-Manning form #1; `facts_deduplicated=2` measured). **Phase 8.6 STRETCH** — bitemporal supersede/coexist 6-action classifier (5/5 dedup-suite tests PASS in 76.5s). 15 BCJ entries (6-15 observed). |
| 3.5.9 | [[Week 3.5.9 - Requirement-Driven Memory Architecture]] | Meta-skill cap of the W3.5 cluster + three-tier hypergraph implementation as worked example (~22h intensive). Teaches requirement → architecture decision via LongMemEval, then graduates to L3 implementation. Phase 1 requirement analysis (decompose each LongMemEval `question_type` into 5-primitive vector; 7-row matrix). Phase 2 architecture decision (apply Phase 1 vs §2 class-capability matrix; 3/1/1 split → HYBRID JUSTIFIED; ≥30% multi-entity-intersection rate documented as L3 graduation trigger). Phase 3 STRETCH Mem0 baseline (`src/mem0_backend_adapter.py` ~80 LOC, lab-Qdrant-shared config for apples-to-apples). Phase 4 STRETCH homebrew hybrid router (`src/atomic_fact_memory.py` per-message extraction + `src/router_memory.py` question-type dispatch — single-write + smart-read avoids dual-write reconciliation). Phase 5 STRETCH N-backend × 7-axis comparison matrix (2 cells from W3.5.8 §7.7 baseline; remaining cells TBD pending Phase 3-9 runs). Phase 6 HyperMem L3 service setup (`:1996` docker-compose alongside L2 EverCore `:1995`). Phase 7 `src/three_tier_memory.py` (~200 LOC, extends TieredMemory with `query_relations()` + envelope translation + question-type routing for temporal-reasoning + multi-entity-intersection axes). Phase 8 extended consolidation pipeline (scrolls → EverCore imprints + HyperMem hyperedges; closed-entity-enum + provenance + canonicalized idempotency keys). Phase 9 6-backend LongMemEval matrix (qdrant / evercore / mem0 / atomic_fact / hybrid_router / three_tier × 7 axes — primary measurement deliverable; predicted L3 win on temporal-reasoning, with honest null-result documentation discipline if it doesn't materialize). |
| 3.5.95 | [[Week 3.5.95 - Self-Observability Memory]] | **PAI v7.6 self-facing memory.** SPEC v0 — phases scoped, code TBD. World-facing memory (W3.5/3.5.5/3.5.8/3.5.9) covers facts about the domain. PAI v7.6 adds the SELF-facing axis: OBSERVABILITY (every tool call + hook firing + satisfaction signal — agent's own behavioral log) + LEARNING (meta-patterns the agent extracted about itself). 5 phases: (1) OBSERVABILITY SQLite schema extending W4 `obs.py`; (2) tool-call instrumentation hooks into W4 ReAct loop; (3) LEARNING extractor — periodic LLM job summarises OBSERVABILITY rows into self-pattern facts; (4) metacognitive recall — pre-decision query against LEARNING + OBSERVABILITY; (5) measurement — paired-trial probe set proving recall changes behaviour. Reflexion (arXiv:2303.11366) as canonical self-reflection prior art. |
| 3.5.96 | [[Week 3.5.96 - Self-Wiring Memory (GBrain)]] | **SPEC+lab v0 — 4th memory class added to W3.5.9 taxonomy (TRENDS-LIFT 2026-05-28).** Markdown-first deterministic-graph (zero LLM calls at write time). GBrain (gbrain.homes, Garry Tan / YC, MIT) — production memory layer at 146K-page scale powering Tan's OpenClaw + Hermes deployments. 5 typed edges (attended/works_at/invested_in/founded/advises) extracted via deterministic regex; HNSW vector + Postgres keyword + RRF (83→95% Recall@5 measured). 74 MCP tools exposed; synthesis layer with citations + explicit "what we don't know" gap-flagging. 6 executable lab phases: install+Postgres / corpus prep / ingest / RRF benchmark / synthesis layer / W7 agent MCP integration. Complements W3.5.9's three-class taxonomy. |
| 3.7 | [[Week 3.7 - Agentic RAG]] | Phase 1-4: LangChain official 5-node Agentic RAG notebook end-to-end + head-to-head vs Week 3 single-pass on 50-Q dev set + CRAG variant + decision tree (when help vs hurt). Phase 6: hand-rolled Self-RAG + CRAG baseline ported from `shaneliuyx/rag` to current stack (Qdrant + oMLX + `shared/rag_hybrid`). Phase 7: query decomposition with topological execution (DAG-based planning, IRCoT-style). Phase 8: FastMCP server wrapper exposing the lab as 3 tools (`rag_query`, `rag_status`, `rag_decompose`) consumable from Claude Desktop / Cursor — first MCP-server pattern in the curriculum. **Soundbite 4** (Interview Q8 cover) — multi-dim query rewriting + user-in-loop clarification. |
| 4 | [[Week 4 - ReAct From Scratch]] | (1) 150-line `react.py` with no framework; (2) 4 local tools (search, repl, read_file, write_file); (3) SQLite trace logging; (4) 15+ bad-case scenarios with a before/after diff per patch; (5) `RESULTS.md` with failure-mode table. **§1.5 MLX Studio gateway refactor** (lab commit 2026-05-05, chapter synced 2026-05-15) — single `:8080/v1` gateway dispatches 7 roles (loop / tool_arg / classify / reason / compose / finisher / hard_loop); `src/models.py` ~150 LOC; JANG_4M-CRACK vs heretic empirical tie-break (tool=1.00 vs tool=0.00). |
| 4.5 | [[Week 4.5 - Model Routing and Effort Tiering]] | **R2 chapter expansion complete — 1172 lines, all 5 Phases R2-shipped 2026-05-15.** Theory ~1500 words with 3 load-bearing properties + 9 references + 4 distinguish-from contrasts. Phase 1 fleet_config + smoke_test. Phase 2 stratified-split probe set (60 hand-labelled prompts, tier × mode). Phase 3 single-classifier dispatch (`router.py` + `tier_dispatch.py`; target ≥0.85 per-tier accuracy). Phase 4 BART-MNLI vote layer (`router_bart.py` + `router_vote.py` asyncio.gather parallel + SQLite disagreement log + safety-bias tie-break). Phase 5 4-way cost-latency bench (`test_four_way_bench.py` with opus_always / classifier / vote / random verdict factories + cost rate card). §6 BCJ + §7 Soundbites + RESULTS.md await Phase 5 real-run population. **Soundbite (d)** — Q9 parallelized-intent-recognition cover. |
| 4.6 | [[Week 4.6 - Durable Agent Runtime and Process Topologies]] | **SPEC v0 — phases scoped, code TBD, 200-300 LOC hard cap.** AutoGPT classic→Platform postmortem motivates "execution state separate from LLM loop". 5 phases: (1) SQLite graph store + persistence; (2) asyncio worker pool consuming queued nodes; (3) `fcntl.flock` POSIX file-based distributed lock; (4) per-node cost meter (token + wall-clock) + observability table with hardcoded Claude Sonnet 4.6 / Haiku 4.5 / Opus 4.5 rate card; (5) trigger-based scheduler (cron + webhook + manual) + 4-topology benchmark (sequential / parallel / hierarchical / workflow per PraisonAI). Temporal.io durable-workflow thesis as the production parallel. |
...
| 5.6 | [[Week 5.6 - ISA-Driven Metacognition]] | **SPEC v0 — PAI v5.0 Ideal-State Artifact as falsifiable termination contract.** Critique of free-form Reflexion (W5.5) — without explicit termination contract, hill-climbing agents loop indefinitely or stop early. 5 phases: (1) ISA YAML schema + Pydantic v2 model (free JSON-schema export for scaffolder LLM); (2) ISA scaffolding from user prompt via haiku-tier LLM (Qwen3.5-9B); (3) ISC verification dispatcher with cascade (mechanical checks → test runs → structured-output → LLM-judge fallback with verdict+reason JSON); (4) hill-climbing loop wrapping W4 ReAct (re-evaluate ISCs after each action); (5) synchronous reconciliation (user feedback mid-run updates ISA + re-plans). |
...
| 6.4 | [[Week 6.4 - Low-Code Agent Platforms]] | **Low-code platform fluency (hello-agents Ch5 cross-ref).** Self-host Dify + ship the W4 ReAct equivalent (4 tools + RAG + memory) via UI only — compare time-to-ship + LOC owned. Coze plugin authoring (multi-modal first-class). n8n workflow automation (trigger-based scheduling first-class). 5-axis low-code-vs-custom decision matrix (audience / iteration speed / deployment surface / compliance / cost ceiling). 4 production failure modes: workflow canvas drift / token cost explosion / tool ABI drift / multi-platform fragmentation. Senior-signal framing: low-code platform fluency + curriculum primitives fluency together = "I know what each Dify node abstracts because I built it from scratch." |
...
| 6.6 | [[Week 6.6 - MCP Schema Bridge]] | **SPEC v0 — type-hint → JSON Schema producer + AsyncGenerator streaming.** Port of PraisonAI's `python_type_to_json_schema` (~100 LOC) as the canonical pattern. 5 phases: (1) helper port; (2) `@mcp_tool` decorator — type hints → input_schema, docstring → description, return type → output_schema; (3) MCP stdio server exposing 5 example tools (calculator, file-search, currency-converter, geo-distance, async-counter for streaming); (4) consumer test — local Hermes on `:8004` dispatches each tool; (5) streaming-output extension — AutoGPT-style `AsyncGenerator[BlockOutputEntry]` for long-running tool. Foundation for W7 tool harness's typed-arg pattern. |
| 6.65 | [[Week 6.65 - MCP Production Transports]] | **SPEC v0 — Streamable HTTP + DNS-rebinding defense + 2026-migration + gateway composition + tool-poisoning.** The 2025-03-26 MCP spec replaced HTTP+SSE two-endpoint with Streamable HTTP single-endpoint (POST + GET + DELETE on /mcp + `Mcp-Session-Id` header). 2025-11-25 spec mandates `Origin` allowlist defeating DNS-rebinding. Deadline-sensitive: Atlassian Rovo (2026-06-30) + Keboola (2026-04-01) removed legacy SSE. 6 lab phases: skeleton / Origin defense / SSE+last-event-id / backwards-compat probe / gateway multiplex / tool-poisoning red-team. Extended with §10 WorkOS auth.md agent-registration protocol (May 2026). Source: Phase 13 lessons 09/15/17 + WorkOS auth.md launch. |
| 6.75 | [[Week 6.75 - Skill Optimization (SkillOpt)]] | **SPEC+lab v0 — text-space optimization of skills (TRENDS-LIFT 2026-05-28).** microsoft/SkillOpt (May 2026, arXiv:2605.23904, MIT). Trains markdown skills with epochs/batchsize/learning-rate + validation-gated updates — WITHOUT touching model weights. Three components: trajectory collector + evaluator + LLM-as-editor optimizer. 6 executable lab phases on SearchQA: install / data prep / 4-epoch training / test eval / edit-trace analysis / deploy to W7 agent. Companion to W6.7 authoring; produces `best_skill.md` artifact deployable to any frozen LLM. |
| 6.7 | [[Week 6.7 - Authoring Agent Skills]] | Three production-quality skills authored end-to-end; SKILL.md + trigger pattern + verification step each; install globally + use for one week; description-field iteration log capturing trigger-tuning. **Soundbite 4** (Interview Q12 cover) — how skills are implemented (markdown + description-loader at session boot). Extended (2026-05-28) with **microsoft/apm — Agent Package Manager** section: `apm.yml` manifest + lockfile + `apm install` + compile targets (Claude Code / Cursor / Copilot / Codex / Gemini / Windsurf / OpenCode). |
...
| 6.85 | [[Week 6.85 - Prompt Template Engineering Patterns]] | **Consolidation chapter (interview Q1 cover); no new lab.** 5-axis design space: persona / task / output schema / few-shot / failure guards. 4 anti-patterns: prompt drift / schema collapse / persona-task confusion / output-format-in-system. §1 audit table of 5 curriculum prompts (W3.5.8 dedup + W4 ReAct + W4.5 router + W4.5 finisher + W3.7 decompose) vs the 5 axes. §2 schema-enforcement-with-retry-once primitive (strip-fence + Pydantic validate + retry-once-with-error). 3 interview soundbites covering "how prompt templates are constructed". |
| 6.9 | [[Week 6.9 - Context Engineering and Todo Mechanisms]] | **Consolidation chapter (interview Q11 cover); no new lab.** 4 context shapes: rolling buffer / summarized history / RAG-recalled / structured-state-as-list. Cognitive-narrowing argument for why todo lists focus models. Context-budget formula in LaTeX: $T_{\text{sys}} + T_{\text{tools}} + T_{\text{hist}} + T_{\text{recall}} + T_{\text{state}} + T_{\text{out}} + T_{\text{margin}} \leq W$ with $T_{\text{out}} \geq 0.25 W$. §1 80-LOC TodoList primitive with 4-state semantics (pending / in_progress / done / blocked). 3 production failure modes: overflow / drift / staleness. |
| 6.95 | [[Week 6.95 - A2A Protocol]] | **SPEC v0 — Agent-to-Agent protocol (Google → Linux Foundation → v1.0 April 2026, 150+ supporters).** Agent Card at /.well-known/agent.json + Task lifecycle (submitted → working → input_required → completed/failed/canceled/rejected) + Messages with mixed Parts (text/file/data) + typed Artifacts + AP2 signed cards (September 2025 extension). Opacity-preservation thesis: cross-org collaboration without revealing internals. §2.5 MCP-vs-A2A decision matrix + 6 lab phases. Timeline absorbs IBM ACP (August 2025) + AP2 payments. Source: Phase 13 lesson 19 + Phase 16 lesson 12 (`rohitg00/ai-engineering-from-scratch`). |
...
| 7.6 | [[Week 7.6 - Small-Model Agent Stacks (MagenticLite + Fara1.5)]] | **SPEC+lab v0 — local-first small-model agent stack (TRENDS-LIFT 2026-05-28).** microsoft/magentic-ui + microsoft/fara (May 2026, Apache 2). MagenticBrain (Qwen3-8B fine-tune) plans/delegates/uses-terminal; Fara1.5-9B (Qwen3.5 base, recommended flagship) handles browser computer-use; Magentic-UI harness orchestrates; Quicksand VM sandbox isolates browser sessions. Thesis: tool-orchestration > knowledge — small models suffice when trained end-to-end with inference tool schemas. Fara1.5-9B ~2× Fara-7B on Online-Mind2Web. 6 executable lab phases: install / Fara1.5 provision / Online-Mind2Web run / Claude-Sonnet benchmark / WebTailBench v2 subset / stack-choice exercise. |
| 7.7 | [[Week 7.7 - Quantization and Inference Optimization]] | **JD#1 cover — quantization + real-time inference + resource-constrained environments.** Bit-width hierarchy FP32 → 2-bit in LaTeX-rendered memory math. KV-cache formula $M_{\text{KV}} = 2 \cdot n_{\text{layers}} \cdot n_{\text{heads}} \cdot d_{\text{head}} \cdot L_{\text{seq}} \cdot b_{\text{KV}}$. Memory-bound vs compute-bound regime via bandwidth formula. PTQ vs QAT vs AWQ / GPTQ / SmoothQuant / MXFP4 comparison. 4 phases: (1) memory audit on local fleet (4 models); (2) perplexity probe across quants on 100-sample multi-domain probe; (3) KV-cache curve + per-token latency vs seq_len; (4) batch-regime sweep classifying memory-bound vs compute-bound. Lab `lab-07-7-quantization` shipped. |
| 7.8 | [[Week 7.8 - Code-Agent Patterns AST Coverage Mocks]] | **Interview Q3-Q7 cover — code-agent skill cluster.** AST + LSP + branch coverage + mock-injection. 4-class testability filter: I/O / decorated / dynamic-dispatch / concurrency. 4 phases: (1) tree-sitter / Python ast walk + 4-flag testability classification; (2) multilspy + pyright LSP integration for cross-file references + inferred types; (3) `coverage.py --branch` JSON + LLM-guided edge-case test generation loop (target 92% from 65% baseline); (4) signature-validated `MagicMock` injection helper. Lab `lab-07-8-code-agent` shipped. |
...
| 8.7 | [[Week 8.7 - Generative Media and Fine-tuning]] | **JD#4 cover — brand-consistent generative media + fine-tuning.** Diffusion forward+reverse process in LaTeX. DDPM / DDIM / latent-diffusion / classifier-free guidance comparison. LoRA math + brand-consistency training pattern (rank 16, target attention modules, ~30 min M5 Pro for 1000 steps on 10-15 reference images). ControlNet + IP-Adapter stack at inference (LoRA = identity, ControlNet = structure, IP-Adapter = style). Eval: CLIP score + FID + human panel necessity. Video extension via Stable Video Diffusion I2V (14-25 frames, 8 fps). Lab `lab-08-7-genmedia` shipped. |
...
| 9.3 | [[Week 9.3 - Agent Performance Evaluation]] | **Agent-specific eval discipline (hello-agents Ch12 cross-ref).** Distinguishes LLM eval (MMLU/GSM8K) vs RAG eval (RAGAS/BEIR) vs AGENT eval (AgentBench/GAIA/SWE-bench/WebArena/OSWorld/τ-bench/MLE-bench). 5-dim per-trajectory rubric: terminated correctly / tool-selection / arg-correctness / no-hallucination / cost reasonable. LLM-as-judge harness with Cohen's κ ≥ 0.6 gate against 20-50 hand-labeled gold trajectories; judge model ≠ agent model (avoid same-model bias). 4 anti-patterns: golden-trajectory rigidity / single-metric optimization / single-benchmark overfit / closed-loop evaluator. Run W4 ReAct on AgentBench-OS subset + Pareto plot pass-rate vs cost-per-task against published GPT-4 baselines (local-MLX ~weaker-than-frontier expected; honest reporting = senior signal). |
...
| 11 | [[Week 11 - System Design]] | 5 × 2-hour whiteboard exercises (doc-QA / multi-agent triage / coding agent / financial-research / infra-aware); self-recorded; **7-point self-critique rubric** (Gate 7 = quotable cost-cut from a prior lab + named routing rule, the offer-closing senior signal). Infra-aware SRE agent #5 composes W11.6 (OTel tracing) + W11.8 (CT pipeline) as primitives. |
| 11.5 | [[Week 11.5 - Agent Security]] | 10-attack red-team suite extending W7 tool harness (5 attack classes + 3 indirect-injection variants); three defense layers (pydantic input validation / firejail sandbox / Llama Guard output filter); kill-rate-per-layer table. Extended (2026-05-27) with bias representational harm + fairness criteria (group/individual/counterfactual + impossibility theorem) + Model Card discipline (Mitchell 2019) + MAST multi-agent failure modes + EchoLeak CVE cross-link. Source: Phase 18 lessons 20/21/26. Further extended (2026-05-28) with **7-layer defense-in-depth stack** adding Layer 5 microsoft/RAMPART (pytest red-team) + Layer 6 denoland/clawpatrol (wire-level firewall via HCL rules) + Layer 7 perplexityai/bumblebee (read-only supply-chain scanner for dev endpoints). |
| 11.55 | [[Week 11.55 - Content Provenance and AI Regulatory]] | **SPEC v0 — content provenance + 4 regulatory frameworks.** Three-tech provenance stack layered for defense-in-depth: SynthID (Google DeepMind token + image + video watermarks) + Stable Signature (ICCV 2023 latent diffusion decoder fine-tune) + C2PA 2.2 (cryptographically signed metadata). 2024 Stable-Signature-removal attack + 2025 meaning-preserving watermark attack as failure-mode anchors. Four regulatory frameworks: EU AI Act (Aug 2026 full applicability + €35M / 7% revenue penalties) / US EO 14110 / UK AISI / Korea AI Basic Act. Model Card / System Card / Dataset Card discipline. EchoLeak CVE class (Microsoft Copilot prompt-injection-via-retrieval). 6 lab phases SPEC. Source: Phase 18 lessons 23/24/25/26/27 (`rohitg00/ai-engineering-from-scratch`). |
| 11.6 | [[Week 11.6 - Production Tracing and Cost Telemetry]] | **JD#2 cover — fine-grained tracing + throughput optimization.** OpenTelemetry instrumentation of W4 ReAct loop + Langfuse self-hosted. Cost formula: $C_{\text{call}} = t_{\text{in}} \cdot p_{\text{in}} + t_{\text{out}} \cdot p_{\text{out}}$. DuckDB rollup queries: cost-per-role-per-day + p99-latency-per-model-per-hour. Why-p99-not-mean argument with heavy-tail example (p99 = 7800ms while mean = 850ms). 4 production failure modes documented (lost spans, wrong token counts, sampling loss, cost drift). Lab `lab-11-6-tracing` shipped. |
...
| 11.8 | [[Week 11.8 - Continuous Training and MLOps Pipelines]] | **JD#3 cover — production ML + CI/CD/CT automation.** CI vs CD vs CT distinction + Google MLOps maturity Levels 0-3. PSI drift detection: $\text{PSI}(P, Q) = \sum_i (p_i - q_i) \ln(p_i/q_i)$ with 3-day-consecutive rule. Eval-gated deployment: $\text{deploy iff cand} \geq \text{prod} - \epsilon$. Shadow ramp 5% → 25% → 100%. 4 phases: (1) drift detector with PSI + traffic-volume guard; (2) eval-gated GitHub Actions CI; (3) shadow router with rollback; (4) glue cron loop. Lab `lab-11-8-ct` shipped. |
...
| 12.5 | [[Week 12.5 - Multimodal Agents (Vision-Language + Cross-Modal RAG)]] | **SPEC v0 — multimodal agent fundamentals + cross-modal RAG.** Three VLM substrate choices: CLIP (contrastive shared-space) / BLIP-2 (Q-Former bridge) / LLaVA (visual instruction tuning). ColPali vision-native document RAG with late-interaction patch embeddings (PaliGemma backbone). Four-stage multimodal RAG pipeline: per-modality retrieval heads / fusion (score / attention / MoE) / generation grounding / multimodal eval. Three 2025 surveys (Abootorabi / Mei / Zhao) shared sub-problem taxonomy. Omni-models thinker-talker pattern (Qwen2.5-Omni, GPT-4o, InternVL3). 6 lab phases SPEC. Source: Phase 12 lessons 01-05/20/23/24 (`rohitg00/ai-engineering-from-scratch`). |
