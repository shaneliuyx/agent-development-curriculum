...
### Week 3.5.9 — Requirement-Driven Memory Architecture (half-week insert, ~16h, ~50% complete)
> Detailed runbook: [[Week 3.5.9 - Requirement-Driven Memory Architecture]] *(SPEC + design fleshed; Phase 3-5 implementation runs pending)*

The meta-skill cap of the W3.5 cluster. W3.5 / W3.5.5 / W3.5.8 taught individual memory architectures (single-agent dual-store, multi-agent shared queue, two-tier consolidation). W3.5.9 teaches the META-SKILL — given a real published benchmark (LongMemEval), analyse the data shape, derive required memory primitives per axis, evaluate three candidate architecture classes (1-tier atomic-fact, 2-tier consolidation, graph-tier temporal), pick the architecture whose write-time primitive matches the read-time question shape, implement, verify. The deliverable a reader walks away with: not memorised backend details, but a defensible decision-making framework + a working hybrid router implementation. The originally-planned "HyperMem three-tier + 5-backend benchmark" chapter (W3.5.9-Hypergraph) is now a **TBD future decimal** — its content lives in W3.5.8's "When to Add a Third Tier" section + a planned future chapter on HyperMem-specific implementation.
...
**Lab (12-14h, ~50% complete) — reuses `lab-03-5-8-two-tier` (no separate lab repo; this chapter extends W3.5.8's eval harness with new backends).**
...
2. **Phase 2 — Architecture Decision (~1h, ANALYSIS COMPLETE)**: apply Phase 1's matrix against the §2 class-capability matrix. Derive per-axis winner. Joint result: 1-tier wins 3/7 axes, 2-tier wins 1/7, graph-tier wins 1/7, 2/7 are ties. 3/1/1 split → HYBRID JUSTIFIED. Architecture choice: 2-backend hybrid with question-type routing (graph-tier scoped out as TBD-future).
...
4. **Phase 4 STRETCH — Homebrew hybrid router (~3h, CODE SHIPPED + RESULTS TBD)**: the chapter's intellectual contribution. `src/atomic_fact_memory.py` (~120 LOC) per-message atomic-fact extraction → Qdrant (one LLM call per message emitting JSON array; one Qdrant point per fact). `src/router_memory.py` (~150 LOC) question-type classifier + dispatch (atomic_fact for IE + multi-session + temporal; tiered_2tier for knowledge-update). Single-write path + smart read routing (deliberately avoids dual-write reconciliation risk).
5. **Phase 5 STRETCH — Compare + Aggregate (~1h, METHODOLOGY DOCUMENTED + RESULTS TBD)**: extend `scripts/aggregate_results.py` for 5-backend × 7-axis × N-question matrix. Sequential not parallel (resource-contention discipline). Axis-wise not aggregate-only (exposes which question types each backend handles). 2 cells pre-populated from W3.5.8 §7.7 measured baseline (qdrant 0/20 + evercore 0/20 on multi-session + knowledge-update); the remaining cells fill in after Phase 3-5 runs.

**Exit criteria.** 90-second answer to *"how do you decide between 1-tier and 2-tier memory?"* — name the requirement-matrix decomposition, the joint-matrix decision rule, the 3/1/1-split-hybrid-justification, your measured per-axis scores (TBD pending Phase 3-5 runs). Bonus: when would you NOT build a router-based hybrid? (answer: when a single class wins ≥6/7 axes — pick the simpler operational shape). The chapter's intellectual claim: 80% of the value comes from getting the WRITE-TIME PRIMITIVE right (per-message atomic-fact for atomic-fact recall); the last 20% requires production-grade retrieval fusion (Mem0 has it natively; the homebrew doesn't yet).

**Infra bridge.** The decision matrix is the SAME shape as a database engineer's "OLTP vs OLAP vs HTAP" decision — read-time access pattern determines write-time storage choice. The single-write + smart-read pattern is the production stand-in for synchronous dual-write — avoids reconciliation contracts (cf. database CDC-vs-dual-write trade-off). The question-type-router-by-explicit-label pattern is the same shape as a load-balancer's host-header-based routing — explicit metadata beats inferred classification at production reliability. The "deliberately scope-cut graph-tier as TBD-future" discipline mirrors the production-architect's habit of "describe what you're NOT building today + why" alongside what you ARE building.
...
| 3.5.9 | [[Week 3.5.9 - Requirement-Driven Memory Architecture]] | Meta-skill cap of the W3.5 cluster. Teaches requirement → architecture decision via LongMemEval as worked exercise. Phase 1 requirement analysis (decompose each LongMemEval `question_type` into 5-primitive vector; 7-row matrix). Phase 2 architecture decision (apply Phase 1 vs §2 class-capability matrix; 3/1/1 split → HYBRID JUSTIFIED). Phase 3 STRETCH Mem0 baseline (`src/mem0_backend_adapter.py` ~80 LOC, lab-Qdrant-shared config for apples-to-apples). Phase 4 STRETCH homebrew hybrid router (`src/atomic_fact_memory.py` per-message extraction + `src/router_memory.py` question-type dispatch — single-write + smart-read avoids dual-write reconciliation). Phase 5 STRETCH 5-backend × 7-axis comparison matrix (2 cells from W3.5.8 §7.7 baseline; remaining cells TBD pending Phase 3-5 runs). Originally-planned HyperMem three-tier chapter is now TBD future decimal — content moved to W3.5.8 "When to Add a Third Tier" section. |
