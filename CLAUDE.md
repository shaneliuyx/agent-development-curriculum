...
1. **Architecture diagram** — Mermaid (non-ASCII) showing the data flow / call graph for THIS script. Place immediately before the code. Mermaid hygiene rules (NORMATIVE):
    ...
    - **Font size directive** — every mermaid block opens with `%%{init: {'theme':'default', 'themeVariables': {'fontSize':'42px'}}}%%` immediately after the ```` ```mermaid ```` fence. Default Obsidian/Mermaid rendering is unreadably small at chapter-zoom; 42px restores a reading-friendly text size across all diagram types (flowchart, sequenceDiagram, etc).
