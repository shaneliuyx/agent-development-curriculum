...
- **Code-defined topology ≠ declarative config-composed topology.** This chapter builds topologies in code (`topologies.py` — four DAG shapes as Python). A declarative system (youtu-agent composes the whole multi-agent graph from YAML via Hydra `defaults` + `${oc.env:}` secret interpolation) trades code flexibility for config-driven extensibility: a new topology is a YAML file, not a code change. Orthogonal to durability; the trade-off (debuggability vs hot-swap extensibility) is Engineering Decision Pattern 47.
...
- **bytedance/deer-flow** — https://github.com/bytedance/deer-flow — a long-horizon "SuperAgent" harness (researches / codes / creates) with sandboxes, subagents, memories, tools, skills, and a message gateway. Source for **three** W4.6 durable-runtime patterns added 2026-06-22: the **subagent status contract + polling-timeout safety-net** (`subagents/status_contract.py` + `task_tool.py` → Phase 6, BCJ Entry 2, [[Engineering Decision Patterns]] Pattern 38), **corruption-safe concurrent artifact writes** (sandbox file operations → Phase 7, BCJ Entry 3, Pattern 42), and **fan-out cost aggregation + parent-level ceiling** (`subagents/token_collector.py` → Phase 8, Pattern 39). Far too large (72k★, full sandbox/gateway stack) to fork for a 250-LOC local-first lab — cited as the production-shape source; the minimal deterministic versions are built in `src/{subagent,artifact_writer,cost_ceiling}.py`. The agentkit shared lib carries the productionised mirrors (`agentkit/runtime/subagent.py`).
