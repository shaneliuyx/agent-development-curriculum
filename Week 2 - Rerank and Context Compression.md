...
*Fix:* Drop hybrid for MS MARCO; switch eval to BEIR-FiQA (ceiling-free, dense recall@100 = 0.61). On FiQA, hybrid showed +0.5 pp lift over dense (recall@10 0.6821 vs 0.6775) — direction reversed but lift is small; the real argument for hybrid on FiQA is qualitative (sparse + dense complementary on financial-jargon queries), not quantitative. **Lesson:** test fusion benefit on ceiling-free corpora; saturated benchmarks invert the signal. Architectural fix is corpus-aware routing, not RRF k tuning.
...
**Soundbite 1 — *"When does hybrid retrieval earn its cost, and when does it not?"***
"Hybrid retrieval is the cheap first stage before you commit to a reranker. RRF fuses dense and sparse rankings without score calibration, and the incremental latency is near-zero because BGE-M3 produces both heads in one forward pass. What I learned is that the lift is corpus-dependent — on a saturated benchmark the hybrid barely moves the needle, while on a harder domain-specific corpus it opens a visible gap. I use the hybrid baseline as a diagnostic: if dense already captures almost everything, the reranker's marginal gain shrinks and may not justify its latency budget."
*Principle:* hybrid lift is corpus-dependent; measure it before buying the reranker.

**Soundbite 2 — *"What's the single biggest lever for reranker latency, and does it generalize across workloads?"***
"The dominant lever is precision: dropping the cross-encoder to fp16 cut latency by about two-thirds and kept recall and nDCG statistically identical. Batch size gave a small additional gain by reducing kernel-launch overhead. What doesn't generalize is cross-query batching — an offline eval packs thousands of queries into one batch, which hides launch overhead in a way production never sees. I measured this explicitly: the fp16 gain transferred cleanly to per-query latency; the batching gain evaporated. The lesson is to benchmark in the shape your production will actually run."
*Principle:* fp16 is the portable lever; batching optimizations only transfer if production shares the same request shape.

**Soundbite 3 — *"You swept chunk sizes — what surprised you?"***
"Smaller chunks beat larger ones by a large margin, which contradicts the intuition that more context is better. The mechanism is averaging: a bi-encoder pools the whole chunk into one vector, so a relevant span occupying a small fraction of a large chunk contributes proportionally little signal. Overlap mattered a lot at small chunk sizes and almost nothing at large ones — which makes sense, because large chunks already span the boundary the overlap is trying to bridge. I wouldn't copy-paste a chunk-size default from a tutorial; I'd sweep the grid on a held-out sample of the target corpus."
*Principle:* average-pooling dilutes sparse relevance — smaller chunks concentrate signal, and overlap value is size-dependent.

**Soundbite 4 — *"In the library refactor you got the same recall numbers but much lower wall-time — how?"***
"The per-query LangChain wrappers are designed for online serving: one query in, one result out, with embedding called on every request. When I switched to a batch-first API — pre-embed all queries, push them in one bulk call — wall-time on retrieval dropped dramatically while recall was bit-for-bit identical. Same model weights, same vectors, completely different throughput. The takeaway is to pick a library by its API shape, not by its benchmark numbers. A method that takes a list of inputs is a throughput tool; a method that takes one string is a serving tool. Conflating them is a common source of unexplained slowness in offline eval pipelines."
*Principle:* API shape determines throughput ceiling; same model in a serving-shaped API will plateau far below a batch-shaped API at eval scale.

**Soundbite 5 — *"How did you validate that aggressive compression was safe to ship?"***
"I ran an LLM-as-judge A/B: two answers per query — one from compressed context, one from raw — and asked the judge which was better or whether they tied. Compressed answers won or tied on a clear majority of queries, beating the safety threshold I'd set in advance. The tricky part was calibration: my first run reported a higher wins-plus-ties rate, but two of the judge calls had failed silently due to reasoning-budget exhaustion, and both failures turned out to be hard queries where compression hurt. Filtering those out inflated the metric. The honest number — after bumping the judge's token budget until zero failures — was lower, still above threshold, but the lesson is that judge failures correlate with case difficulty, so `n_ok`-only metrics overstate quality."
*Principle:* LLM-as-judge quality gates require zero-failure judge runs; survivorship bias from silent judge failures systematically inflates pass rates.
...
- **Cited by:** chapters that reference this chapter as a prerequisite or build-on; reverse links per Pattern 21 (Bidirectional Cross-Reference Invariant):
  - **W0.5**: LLM Internals — reranker architecture (cross-encoder vs ColBERT) is the per-token-internals view of W2's reranking primitive
