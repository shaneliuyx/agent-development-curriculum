...
"Hybrid retrieval is the cheap-first alternative to reranking — BGE-M3's dense and sparse heads come from one forward pass, RRF fuses the rankings without score calibration, and you pay maybe 3-5 ms over dense-only. On BEIR-FiQA my measured lift was +0.5 pp recall@10 (hybrid 0.6821 vs dense 0.6775) — small. The reranker on the same corpus delivered ~10× the lift at maybe 20× the latency. The hybrid baseline isn't here to replace the reranker; it tells me whether the cross-encoder's marginal gain is worth the budget for *this* corpus, and on FiQA the answer is yes — the reranker dominates because the hybrid lift is too small to defend on its own."
