...
- **Distinguish from:** [[Week 6.65 - MCP Production Transports]] — W6.65 is the *tool-call* transport (MCP Streamable HTTP between agent and tool server); W7.3 is the *LLM-call* transport (gateway between agent and model provider). Different hops on the same request path; don't conflate the MCP session layer with the provider gateway layer.
