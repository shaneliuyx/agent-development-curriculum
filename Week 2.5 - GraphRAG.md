...
## Production Design — Iterative Lessons (v1 → v12.4m)
...
| Bucket        | Target                         | v9.5 dense (sub) | v10 broken (sub) | v10c FIX2 (sub) | v10d FIX2 (jud)           | v11 all-fixes (jud)  | v12 QID (jud) | v16 shipped (jud) | v21 final (jud) | v23 eval fix (jud) | **v12.4m universal (jud)** |
| ------------- | ------------------------------ | ---------------- | ---------------- | --------------- | ------------------------- | -------------------- | ------------- | ----------------- | --------------- | ------------------ | -------------------------- |
| factoid       | ≥ 0.80                         | 0.86             | 0.57             | **0.86**        | **0.86**                  | **0.86**             | 1.00          | **1.00**          | **1.00**        | **1.00**           | **1.00**                   |
| two_hop       | ≥ 0.60                         | 0.60             | 0.29             | **0.75**        | **0.75**                  | **0.79**             | 0.71          | **0.75**          | **0.75**        | **0.75**           | **0.97**                   |
| relational    | ≥ 0.70 (graph's signature win) | 0.75             | 0.00             | **0.75**        | 0.75                      | **1.00** ← perfect   | 0.50†         | **0.75***         | **0.75***       | **0.75**           | **1.00**                   |
| multi_hop     | ≥ 0.50                         | 0.29             | 0.10             | 0.17            | 0.21                      | 0.25 (extr. ceiling) | 0.29          | **0.36**          | ~0.34           | **0.65**           | **0.88**                   |
| out_of_domain | refusal ≥ 0.95                 | 0.25             | 0.25             | 0.25            | **1.00** (sub undercount) | **1.00**             | **1.00**      | **1.00**          | **1.00**        | **1.00**           | **1.00**                   |
| **ALL**       | —                              | **0.55**         | **0.25**         | **0.54**        | **0.63**                  | **0.68**             | 0.64          | **0.71**          | **0.70**        | **0.80**           | **0.96**                   |
| W/L/T vs Vec  | —                              | —                | —                | —               | —                         | —                    | —             | —                 | 24/0/8          | 28/0/4             | **32/0/0**                 |
| Avg latency   | —                              | —                | —                | —               | —                         | —                    | —             | 4.2s              | 4.6s            | 5.2s               | ~18s (CoT cost)            |

The v10 → v10c → v10d → v11 → v12 → v16 → v21 → v23 → **v12.4m** trajectory is the empirical anchor for **Best Practices 13 (1-hop priority Cypher), 14 (LLM-judge eval), 15 (fp16 on MPS), 16 (multi-hop query decomposition), 17 (canonical-ID linking at extraction time), 18 (context ordering / "lost in the middle"), 19 (query-type router with PPR fallback), 20 (single-model serving), 21 (temperature=0.0 eval stability), 22 (reasoning-model anti-pattern), 23 (eval-set quality), 24 (universal mechanisms over hardcoded patterns), 25 (mechanism probes before outcome eval), and 26 (LLM-decided flags > hardcoded keyword detection)** below — see [[#Bad-Case Journal]] entries 7–11 for the architectural lessons. The 0.55 → 0.25 → 0.55 → 0.63 → 0.68 → 0.64 → 0.71 → 0.70 → 0.80 → **0.96** path captures the "forward-fix vs revert" decision pattern, the "extraction-completeness vs retrieval-strategy" diagnostic axis, the "data-source choice beats post-hoc fix" principle, and the "universal-mechanism overhaul beats incremental fixes" principle in the soundbites. v11 closed three follow-ups from v10d: index lifecycle (commit `a44f3c8`), multi-hop decomposition (`c038b4a`), reverse-direction triple normalization (`2c1538d`). v12 closes Leak #4 (entity surface-form fragmentation) via Wikidata QID linking — `"Reid Hoffman"` and `"Reid Garrett Hoffman"` collapse to Q211098, restoring multi-hop chains v11 broke at alias boundaries. v16 adds context reordering (step-2-first) and expanded edge_filter, lifting multi_hop 0.24→0.36 (W/L/T 25/0/7). v21 confirms Gemma-4-26B single-model as the stable shipped baseline: ALL jud=0.70, 4.6s latency, W/L/T=24/0/8. v22/v23 close the eval-question design gap (BP 23): replacing corpus-absent expected entities + rewriting implicit seed phrases lifts multi_hop 0.34→0.65, ALL 0.70→0.80, W/L/T 24/0/8→28/0/4 — zero code changes. **v12.4m is the post-ship overhaul that branched off v12.1 with all v13–v23 lessons folded into a unified universal-mechanism framework.** Six core mechanisms in Phase A (`v12.3` — topology gate, two-stage seed resolution, GDS projection lifecycle, QID inline tags, bridge-first ordering, two-pass FACTS+ANSWER output) plus opt-in `expand_terminal` decomposition in Phase B (`v12.4`) plus universal prompts (`v12.4m` — corpus-portable, no hardcoded patterns) lift multi_hop 0.65→0.88, relational 0.75→1.00, ALL 0.80→0.96, W/L/T 28/0/4→**32/0/0** (strict dominance over VectorRAG). Cost: latency 5.2s → 18s due to CoT output tokens — acceptable for batch eval, reconsider for low-latency serving. †v12 relational=0.50 is a temporary rebuild regression (QID graph restructuring); v13 restored 1.00.
...
### Best practices — distilled from v1 → v12.4m
...
24. **Universal mechanisms beat hardcoded patterns when the gain compounds.** Each fix in v13–v23 was a targeted patch (this query type, this edge filter, this entity class). v12.4m re-derived all of them from signals already in the data — `qid IS NOT NULL OR degree >= 2` as a topology gate, two-stage `phrase→OR` seed fallback, `bridge-first` context ordering, `FACTS + ANSWER` two-pass output structure — and produced +0.16 ALL recall (0.80→0.96) on top of v23's eval-repaired baseline. Universal mechanisms are corpus-portable (no domain-specific entity lists, no per-question regex), survive graph rebuilds, and eliminate the per-failure-mode patch-creep that was accumulating in v13–v22. The principle: when ≥3 hardcoded patches solve the same class of problem, redesign with a universal signal that subsumes them. Verified on this lab — 13 universal mechanisms in v12.4m replace ~25 hardcoded behaviors accumulated v10c–v22.

25. **Mechanism probes are upstream of outcome eval.** Outcome metrics (judge ALL recall) hide which sub-mechanism is broken. v12.4m introduced `decomp_probes.py` — 14 known-input/known-output assertions on the decomposition classifier and step-1 intermediate resolution, each independently testable. Pass rate 0.929 gates production deploys. Without per-mechanism tests, you cannot distinguish "topology gate underfilters" from "seed resolution overfilters" from "bridge ordering didn't fire" — they all manifest as the same 0.00 ALL score on the eval. Run mechanism probes before outcome eval; fix at the level the probe localizes; verify with the outcome eval. The discipline generalizes: any retrieval pipeline with ≥3 stages should have stage-level golden-input tests independent of the end-to-end metric.

26. **LLM-decided flags beat hardcoded keyword detection for branching logic.** v12.3 always-on step3 expansion regressed simple chains by flooding context with off-topic edges. v12.4 fixed it by adding an `expand_terminal: bool` field to the decomposition plan — the LLM decides per-query whether the third hop is needed (`"founded by Stanford alumni who later joined VCs"` → true; `"who founded PayPal"` → false). Single LLM call per decision (chain vs intersection, expand_terminal, COMPOUND vs LIST), structured JSON output, deterministic at temperature=0. The pattern generalises: when branching logic depends on natural-language input semantics, an LLM flag in the structured plan beats a regex / keyword-list dispatch. Costs one extra LLM call per query (~0.3s) and recovers the per-query specificity that fixed-rules dispatch cannot.
...
> The six entries below (numbered 6–11 to preserve historical citation continuity) cover *architectural* failure modes that surfaced during the v9–v12 iteration cycle: non-determinism from reasoning-model token-budget exhaustion, predicate fragmentation + query-time aggregation, fulltext-index lifecycle, eval scoring, extraction completeness, and entity-resolution tool selection. Entries 1, 3, 4, 5 (entity duplication illustrative example, hallucinated-edge example, 2-hop-flood example, dev-vs-user mismatch example) were removed in the 2026-05-07 audit pass — they were pre-experiment illustrative scenarios, not anchored to observed events in the lab's `RESULTS.md`. **Entry 2 was consolidated into Entry 7** in the 2026-05-06 dedup pass — both originally documented predicate-vocabulary failures, and Entry 7's open-vocab + query-time-consolidation principle subsumes Entry 2's closed-vocab alternative. For *operational* failures encountered running this lab end-to-end (deprecated dataset loaders, reasoning-model token-budget exhaustion, prompt restrictiveness, fuzzy-match brittleness, the 5-layer corpus-mechanism cascade), see [[#Lab Run-Through Bad Cases (2026-04-30)]] above.
...
**Soundbite 1 — *"When does GraphRAG actually beat vector retrieval, and what's the cost?"***
"GraphRAG wins on questions that require chaining facts across documents — relational, multi-hop, and out-of-domain refusals — because a Cypher traversal follows typed edges, while cosine similarity matches passage tokens. I ran a head-to-head on a closed tech-bio corpus and measured strict dominance on every question category that requires cross-document reasoning. Vector retrieval collapsed to near-zero on those same categories because the answers don't appear lexically in any single chunk. The structural cost is roughly an order-of-magnitude latency increase per query, plus a graph database dependency. So in production I'd route by question shape: cross-document and relational → Graph; factoid lookup → Vector."
*Principle:* GraphRAG wins where the answer lives in the edges, not in the passages.

**Soundbite 2 — *"What's the hardest part of building a GraphRAG pipeline in practice?"***
"The hardest part is keeping the graph clean at extraction time. I had two surface forms of the same person extracted as separate nodes — the formal name from one article, the short form from another — which silently broke every multi-hop chain passing through that entity. My first instinct was embedding-similarity clustering to merge aliases. That produced catastrophic false positives: unrelated people with similar first names merged at high cosine scores because the embedding model was trained on passages, not bare entity names; cosine on three-token strings captures stylistic shape, not referential identity. The correct fix was a knowledge-base lookup — resolving names to canonical Wikidata IDs at extraction time so aliases MERGE to one node by construction. The lesson generalised: entity resolution is a lookup problem, not an embedding problem."
*Principle:* when the embedder gives nonsense on a sub-task, change tools — don't tune the threshold.

**Soundbite 3 — *"How would you deploy GraphRAG in a production system with mixed query traffic?"***
"I wouldn't deploy Graph as the only retriever. The latency gap between graph traversal and vector search is large enough that pure-Graph routing is economically unviable at meaningful QPS. I'd run a shape-aware router — a lightweight classifier or a single cheap LLM call — that categorises each query and routes by question type. Cross-document bridges, relational questions, and out-of-domain refusals go to Graph; factoid lookups on unstructured corpora go to Vector. My lab eval confirmed the categories where Graph's quality advantage is large enough to justify the latency cost, and the categories where it isn't. The architectural win is in the routing logic, not in picking one retriever."
*Principle:* pure-GraphRAG routing is a latency trap; the production pattern is shape-aware dispatch.

**Soundbite 4 — *"You built the corpus from scratch. What surprised you?"***
"GraphRAG quality turned out to be a corpus-engineering problem more than a retrieval-algorithm problem. My first corpus slice was the default row-order cut of a Wikipedia dataset — which happened to be mostly chemistry articles. The graph had zero tech founders. Fixing that required a layered series of repairs: domain-bounded category walk, alphabet-spanning pagination, pageview-weighted sampling to counteract Wikipedia's heavy-tailed attention structure, and two separate pagination bugs in the MediaWiki API that each silently zeroed a fraction of the weights. Every layer masked the previous layer's symptom. The principle I took from it: before debugging retrieval, audit whether the corpus actually contains the entities and facts the questions require."
*Principle:* production retrieval pipelines fail at the corpus layer more often than at the algorithm layer.

**Soundbite 5 — *"How does your entity seed resolution work, and why does precision matter more than recall there?"***
"I use a phrase-first contract at seed resolution: multi-word entity names run as a conjunction requirement against the fulltext index — every named token must match, though not adjacently. If that returns at least one hit, traverse from there. If it returns nothing, fall back to a disjunction with a low-confidence warning. The precision difference between these two modes is dramatic. A conjunction on a person's full name returns the canonical entity node. A disjunction returns every node containing either token — a large fan-out of unrelated people who share a first name. Each spurious seed expands the subgraph with irrelevant edges; the answer hedges or hallucinates. Recall is cheap at seed resolution; precision is what makes the traversal trustworthy. The fallback exists for genuine match failures, not for routine queries."
*Principle:* at graph entry, a high-precision seed beats a high-recall one — spurious seeds corrupt the traversal.

**Soundbite 6 — *"How did you handle predicate fragmentation — open vocabulary versus closed vocabulary?"***
"Open vocabulary extraction produces a dozen surface-form variants for the same semantic relation. The standard production fix is a closed vocabulary at extraction time, which Microsoft GraphRAG and Neo4j's LLM KG Builder both use. The cost is permanent: once you merge `founded` and `co-founded` at extraction, the graph can no longer distinguish solo founding from joint founding. I kept an open vocabulary and instead applied directed deduplication and a consolidation prompt at query time — the LLM is instructed to merge multiple edges between the same pair into one prose sentence with all relations and sources cited. I measured relational questions hitting perfect recall after that fix, with correct canonical direction on all four pairs tested. The principle: open vocabulary is sustainable if aggregation preserves edge direction and the consolidation happens at the prompt layer, not the storage layer."
*Principle:* open-vocab + prompt-layer consolidation preserves semantic nuance that closed-vocab destroys permanently.

**Soundbite 7 — *"A refactor regressed your eval score significantly. What did you do?"***
"My first instinct when eval regressed was to revert. I didn't. I instrumented the LLM context to dump the actual subgraph being passed, found the slicing was discarding canonical one-hop edges before aggregation, and then designed a two-pass Cypher: one-hop priority fill first, multi-hop fill second. Replaced undirected pair-buckets with directed-edge dedup. Added a consolidation prompt. That forward path recovered to baseline and exceeded it on the cross-document category. The principle I'd carry to any team: revert is a fallback for when forward-fix doesn't work, not the default move. Auto-reverting destroys the diagnostic information that the regression just handed you."
*Principle:* a regression is diagnostic signal — forward-fix extracts it; revert discards it.

**Soundbite 8 — *"How do you know your eval metric is actually measuring what matters?"***
"I learned this the hard way. Substring match scored a semantically correct answer zero because the system produced a formal name variant and a synonym for the expected predicate. Adding an LLM judge that explicitly accepts semantic equivalents changed the apparent result substantially — what looked like a large Graph-over-Vector margin under substring scoring became a near-tie under the judge. The honest number flipped my architecture conclusion. From that point forward I ran both scorers on every eval: substring for fast iteration, LLM judge for the decision that actually matters. Eval-set design and scorer design are co-equal concerns — surface-form bias in a substring metric rewards phrasing over correctness."
*Principle:* a metric that encodes surface-form bias will give you the wrong architecture signal.

**Soundbite 9 — *"What did fixing three bugs in one batch teach you about GraphRAG failure modes?"***
"The diagnostic value wasn't the fixes — it was what remained broken after them. I resolved an index lifecycle bug, added query decomposition for bridge questions, and normalised reversed-direction triples in one version. Relational recall hit perfect. But the multi-hop category barely moved — most of the stuck questions stayed at zero. Tracing those failures showed the problem was upstream of retrieval entirely: the bridging edges simply don't exist in the graph because the source text expresses the connection indirectly, across mergers and role changes that the extraction prompt doesn't capture as a direct triple. The lesson: in GraphRAG, the retrieval-versus-extraction diagnostic axis is the first thing to check. Once retrieval is sound, every remaining failure is an extraction problem."
*Principle:* once retrieval is sound, every remaining failure is upstream — in extraction or in the corpus.

**Soundbite 10 — *"What's the generalised lesson from using Wikidata QIDs for entity resolution?"***
"The generalised lesson is that entity resolution belongs at the data layer, not the query layer. Attempting it at query time with embedding similarity produced false positives that no threshold tuning could fix, because the signal being measured — cosine distance on bare entity name embeddings — doesn't capture referential identity. Wikidata QIDs solved it structurally: two names that map to the same QID are the same entity by definition, and the MERGE happens at extraction time so the problem never reaches retrieval. The same principle holds in other domains: enterprise GraphRAG should link to employee IDs, legal GraphRAG to citation IDs, medical GraphRAG to ontology codes. When the source data already carries canonical identifiers, entity resolution stops being a runtime problem and becomes a schema property."
*Principle:* canonical-ID linking at extraction time makes entity resolution a schema property, not a runtime problem.
