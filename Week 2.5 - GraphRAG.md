...
"GraphRAG wins specifically on relational, multi-hop, and refusal-shaped queries — questions whose answers require chaining facts across documents, or that have no answer in the corpus at all. We measured this on a 32-question categorized eval against a 400-article tech-bio Wikipedia corpus, LLM-judge scoring. v12.4m current state: Graph ALL jud 0.96 vs Vector 0.06, W/L/T 32/0/0 in Graph's favor — strict dominance, zero losses. Category breakdown: factoid 1.00 vs 0.00 (vector answers general-knowledge from weights, not the closed corpus); relational 1.00 vs 0.12 — Graph traverses entity-pair edges natively (Apple↔Pixar, Tesla↔SpaceX); two_hop 0.97; multi_hop 0.88 vs 0.03 — Graph chains across articles, Vector cannot; out_of_domain refusal 1.00 vs 0.17. The mechanism is structural — cosine similarity matches passages, but a Cypher query traverses relations. The trade-off: ~18s Graph vs 0.9s Vector (~20× latency, mostly CoT output tokens), with a Neo4j operational dependency. v12.4m's +0.16 lift over the v23 shipped baseline (0.80 → 0.96) came from re-deriving every read-path patch from v13–v23 as a universal mechanism — topology gate, two-stage seed resolution, bridge-first context ordering, FACTS+ANSWER two-pass output, opt-in step3 expansion. Thirteen mechanisms replace ~25 hardcoded patches, and they survive graph rebuilds because they read signals already in the data (QID, degree, BM25). The interview-relevant point: route by question shape. Relational and cross-document bridges → Graph. Factoid lookups where the document contains the answer inline → Vector. Out-of-domain refusals → Graph (fires cleanly). Mixed traffic → HybridRAG router."
...
"In production I'd never deploy GraphRAG as the only retriever despite Graph winning the eval — Graph's ~18 s wall-clock per query vs Vector's ~0.9 s makes pure-Graph routing economically unviable at any QPS above ~3/sec. I'd run a router — a small classifier or a single LLM call — that categorizes queries by shape and routes by latency budget. The 32-Q v12.4m eval shows where Graph's win is large enough to justify the latency: relational (1.00 vs 0.12 judge — Graph traverses entity-pair direction natively); out_of_domain refusal (1.00 vs 0.17 — Graph's precondition warning fires when seed matches zero entities, Vector hedges); factoid (1.00 vs 0.00 — Vector returns 'insufficient context' on this structured corpus because gold answers don't appear in chunks); multi_hop (0.88 vs 0.03 with Wikidata QID linking closing the entity-fragmentation ceiling). For paraphrase / similarity queries on unstructured corpora outside this eval, Vector remains the cheap default. This converges with HybridRAG (Sarmah et al. 2024 arXiv:2408.04948) and Microsoft GraphRAG's published guidance: pure GraphRAG has a structural cost ceiling at production QPS, and the architectural win is in shape-aware routing, not in picking one retriever."
